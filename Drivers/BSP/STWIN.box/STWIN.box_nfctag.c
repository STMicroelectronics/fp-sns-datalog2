...
uint8_t BSP_NFCTAG_isInitialized( uint32_t Instance )
{
  uint8_t status;
  UNUSED(Instance);
   status = (uint8_t)NfcTagObj.IsInitialized;
   return status;
}
...
int32_t BSP_NFCTAG_GPIO_Init(void)
{
  ...
  BSP_NFCTAG_GPO_CLK_ENABLE();

  GPIO_InitStruct.Pin =  BSP_NFCTAG_GPO_PIN;
...
uint32_t BSP_NFCTAG_GetByteSize( uint32_t Instance )
{
  ...
  (void)ST25DV_ReadMemSize(&NfcTagObj, &mem_size );
...
int32_t BSP_NFCTAG_ChangeITPulse(uint32_t MsbPasswd,uint32_t LsbPasswd,const ST25DV_PULSE_DURATION ITtime)
{
  ...
  (void)BSP_NFCTAG_PresentI2CPassword( BSP_NFCTAG_INSTANCE, Passwd );
...
int32_t BSP_NFCTAG_ChangeI2CPassword(uint32_t MsbPasswd,uint32_t LsbPasswd)
{
  ...
  ST25DV_PASSWD Passwd;
  (void)BSP_NFCTAG_ReadI2CSecuritySession_Dyn( BSP_NFCTAG_INSTANCE, &i2csso );
