...
/**
  * @brief This event is generated on the slave when a ACI_GAP_SLAVE_SECURITY_REQUEST is called to reestablish
  *        the bond with a master but the master has lost the bond. When this event is received, the upper
  *        layer has to issue theACI_GAP_ALLOW_REBOND command in order to allow the slave to continue the
  *        pairing process with the master.
  *        On the master this event is raised when ACI_GAP_SEND_PAIRING_REQUEST is called to reestablish a bond
  *        with a slavebut the slave has lost the bond. In order to create a new bond the master has to launch
...
void aci_gatt_attribute_modified_event(uint16_t connection_handle,
                                       ...
                                       uint8_t attr_data[]);
/**
  * @brief This event is generated by the client/server to the application on a GATT timeout (30seconds).
  ...
  *        is closed and no more GATT communication can be performed.
...
/**
  * @brief This event is generated in response to an Exchange MTU request (local or from the peer).
  *        See @ref aci_gatt_exchange_config.
...
/**
  ...
  *        is greater than maximum ATT MTU (on stack versions below v2.1 this event is generated when at
  *        least 2 packets with MTU of 23 bytes are available).
...
/**
  ...
  *        If the reason code is a system crash, a following event @ref aci_blue_crash_info_event will
  *        provide more information regarding the system crash details.
...
/**
  ...
  *        Information provided includes type of radio activity and absolute time in system ticks when a new radio
  *        activity is schedule, if any. Application can use this information to schedule user activities synchronous
  *        to selected radio activities. A command @ref aci_hal_set_radio_activity_mask is provided to enable radio
  *        activity events of user interests, by default no events are enabled.
  *        User should take into account that enabling radio events in application with intense radio activity could
  *        lead to a fairly high rate of events generated.
  *        Application use cases includes synchronizing notification with connection interval, switching  antenna at
