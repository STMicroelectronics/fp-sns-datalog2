...
/**
  ...
  *        failure for the controller. This event is used to notify the Host that a hardware failure has
...
/**
  ...
  * @param status For standard error codes see Bluetooth specification, Vol. 2, part D. For proprietary error
  *        code refer to Error codes section
...
/**
  ...
  * @param status For standard error codes see Bluetooth specification, Vol. 2, part D. For proprietary
  *        error code refer to Error codes section
  * @param connection_handle Connection handle to be used to identify the connection with the peer device.
  * Values:
  *        - 0x0000 ... 0x0EFF
  * @param conn_interval Connection interval used on this connection.
...
/**
  ...
  *        Controller will send on this connection (connEffectiveMaxTxOctets defined in [Vol 6] Part B Section 4.5.10).
  *        Range 0x001B-0x00FB (0x0000 - 0x001A and 0x00FC - 0xFFFF Reserved for future use)
  * @param max_tx_time The maximum time that the local Controller will take to send a Link Layer Data Channel PDU on
  *        this connection (connEffectiveMaxTx-Time defined in [Vol 6] Part B, Section 4.5.10).
  *        Range 0x0148-0x0848 (0x0000 - 0x0127 and 0x0849 - 0xFFFF
  *        Reserved for future use)
  * @param max_rx_octets The maximum number of payload octets in a Link Layer Data Channel PDU that the local controller
  *        expects to receive on this connection (connEfectiveMaxRxOctets defined in [Vol 6] Part B, Section 4.5.10).
  *        Range 0x001B-0x00FB (0x0000 - 0x001A and 0x00FC - 0xFFFF Reserved for future use)
  * @param max_rx_time The maximum time that the local Controller expects to take to receive a Link Layer Data
  *        Channel PDU on this connection (connEffectiveMax-RxTime defined in [Vol 6] Part B, Section 4.5.10).
...
/**
  ...
  *        in order to show the Numeric Value generated, and to ask for Confirmation to the User.
...
/**
  ...
  *        seconds). This is a critical event that should not happen during normal operating conditions.
  *        It is an indication of either a major disruption in the communication link or a mistake in the
  *        application which does not provide a reply to GATT procedures. After this event, the GATT channel
  *        is closed and no more GATT communication can be performed. The applications is expected to issue
  *        an @ref aci_gap_terminate to disconnect from the peer device. It is important to leave an 100 ms
  *        blank window before sending the @ref aci_gap_terminate, since immediately after this event, system
...
/**
  ...
  * @param handle_uuid_pair A sequence of handle-uuid pairs.
...
/**
  ...
  *        A concatenation of attribute values for each of the attribute handles in the request
...
/**
  ...
  *        A sequence of attribute handle, end group handle, attribute value tuples: [2 octets for Attribute Handle,
  *        2 octets End Group Handle, (attribute_data_length - 4 octets) for Attribute Value]
  * @retval None
  */
...
/**
  ...
  *        is greater than maximum ATT MTU (on stack versions below v2.1 this event is generated when at least
  *        2 packetswith MTU of 23 bytes are available).
...
/**
  * @brief 'This event is generated when the device completes a radio activity and provide information when a new
  *         radio activity will be performed.
  *         Informtation provided includes type of radio activity and absolute time in system ticks when a new radio
  *         activity is schedule, if any. Application can use this information to schedule user activities synchronous
  *         to selected radio activitities. A command @ref aci_hal_set_radio_activity_mask is provided to enable radio
  *         activity events of user interests, by default no events are enabled.
  *         User should take into account that enablinng radio events in application with intense radio activity could
  *         lead to a fairly high rate of events generated.
  *         Application use cases includes synchronizing notification with connection interval, switiching antenna at
  *         the end of advertising or performing flash erase operation while radio is idle.
