...
static ble_char_object_t ble_char_gesture_navigation;

/************************************************************
  * Callback function prototype to manage the notify events *
  ***********************************************************/
__weak void notify_event_gesture_navigation(ble_notify_event_t event);

/* Private functions prototype -----------------------------------------------*/
static void attr_mod_request_gesture_navigation(void *ble_char_pointer, uint16_t attr_handle, uint16_t offset,
...
/**
  ...
  * @param  None
  * @retval ble_char_object_t* ble_char_pointer: Data structure pointer for Gesture Navigation service
  */
ble_char_object_t *ble_init_gesture_navigation_service(void)
{
  /* Data structure pointer for BLE service */
  ble_char_object_t *ble_char_pointer;

  /* Init data structure pointer for Gesture Navigation info service */
  ble_char_pointer = &ble_char_gesture_navigation;
  memset(ble_char_pointer, 0, sizeof(ble_char_object_t));
  ble_char_pointer->attr_mod_request_cb = attr_mod_request_gesture_navigation;
  COPY_GESTURE_NAVIGATION_CHAR_UUID((ble_char_pointer->uuid));
  ble_char_pointer->char_uuid_type = UUID_TYPE_128;
  ble_char_pointer->char_value_length = 2 + 1 + 1; /* 2 byte timestamp, 1 byte Gesture Code, 1 byte Button Code */
...
/**
  ...
  */
ble_status_t ble_gesture_navigation_update(ble_gesture_navigation_t gesture_navigation,
                                           ble_button_navigation_t button_navigation)
{
  ...
  buff[2] = (uint8_t)gesture_navigation;
  buff[3] = (uint8_t)button_navigation;

  ret = ACI_GATT_UPDATE_CHAR_VALUE(&ble_char_gesture_navigation, 0, 2 + 1 + 1, buff);

  if (ret != (ble_status_t)BLE_STATUS_SUCCESS)
  {
    if (ble_std_err_service == BLE_SERV_ENABLE)
    {
      bytes_to_write = (uint8_t)sprintf((char *)buffer_to_write, "Error Updating Gesture Navigation Char\n");
...
/**
  ...
  * @retval None
  */
static void attr_mod_request_gesture_navigation(void *void_char_pointer, uint16_t attr_handle, uint16_t offset,
                                                uint8_t data_length, uint8_t *att_data)
{
  if (att_data[0] == 01U)
  {
    notify_event_gesture_navigation(BLE_NOTIFY_SUB);
  }
  else if (att_data[0] == 0U)
  {
    notify_event_gesture_navigation(BLE_NOTIFY_UNSUB);
  }
...
__weak void notify_event_gesture_navigation(ble_notify_event_t event)
{
  ...
  {
    BLE_MANAGER_PRINTF("\r\nNotify gesture navigation function not defined (It is a weak function)\r\n");
  }

  /* NOTE: This function Should not be modified, when the callback is needed,
           the notify_event_gesture_navigation could be implemented in the user file
   */
}
