/**
  ******************************************************************************
  * @file    ble_audio_source_localization.c
...
static ble_char_object_t ble_char_audio_source_localization;

/************************************************************
  * Callback function prototype to manage the notify events *
  ***********************************************************/
__weak void notify_event_audio_source_localization(ble_notify_event_t event);

/* Private functions prototype -----------------------------------------------*/
static void attr_mod_request_audio_source_localization(void *ble_char_pointer, uint16_t attr_handle, uint16_t offset,
...
/**
  ...
  * @param  None
  * @retval ble_char_object_t* ble_char_pointer: Data structure pointer for Audio Source Localization service
  */
ble_char_object_t *ble_init_audio_source_localization_service(void)
{
  /* Data structure pointer for BLE service */
  ble_char_object_t *ble_char_pointer;

  /* Init data structure pointer for Audio Source Localization info service */
  ble_char_pointer = &ble_char_audio_source_localization;
  memset(ble_char_pointer, 0, sizeof(ble_char_object_t));
  ble_char_pointer->attr_mod_request_cb = attr_mod_request_audio_source_localization;
  COPY_AUDIO_SOURCE_LOCALIZATION_CHAR_UUID((ble_char_pointer->uuid));
...
/**
  ...
  * @retval None
  */
void ble_set_audio_source_localization_advertise_data(uint8_t *manuf_data)
{
...
/**
  ...
  */
ble_status_t ble_audio_source_localization_update(uint16_t angle)
{
  ble_status_t ret;
  uint8_t buff[2 + 2];
  ...
  ret = ACI_GATT_UPDATE_CHAR_VALUE(&ble_char_audio_source_localization, 0, 2 + 2, buff);

  if (ret != (ble_status_t)BLE_STATUS_SUCCESS)
  {
    if (ble_std_err_service == BLE_SERV_ENABLE)
    {
      bytes_to_write = (uint8_t)sprintf((char *)buffer_to_write, "Error Updating Audio Source Localization Char\n");
...
/**
  ...
  * @retval None
  */
static void attr_mod_request_audio_source_localization(void *void_char_pointer, uint16_t attr_handle, uint16_t offset,
                                                       uint8_t data_length, uint8_t *att_data)
{
  if (att_data[0] == 01U)
  {
    notify_event_audio_source_localization(BLE_NOTIFY_SUB);
  }
  else if (att_data[0] == 0U)
  {
    notify_event_audio_source_localization(BLE_NOTIFY_UNSUB);
  }
...
__weak void notify_event_audio_source_localization(ble_notify_event_t event)
{
  ...
  {
    BLE_MANAGER_PRINTF("\r\nNotify audio source localization function not defined (It is a weak function)\r\n");
  }

  /* NOTE: This function Should not be modified, when the callback is needed,
           the notify_event_audio_source_localization could be implemented in the user file
   */
}
