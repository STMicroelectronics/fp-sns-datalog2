...
static ble_char_object_t ble_event_counter;

/************************************************************
  * Callback function prototype to manage the notify events *
  ***********************************************************/
__weak void notify_event_event_counter(ble_notify_event_t event);

/* Private functions prototype -----------------------------------------------*/
static void attr_mod_request_event_counter(void *ble_char_pointer, uint16_t attr_handle, uint16_t offset,
...
/**
  ...
  */
ble_char_object_t *ble_init_event_counter_service(void)
{
  /* Data structure pointer for BLE service */
  ble_char_object_t *ble_char_pointer;

  /* Init data structure pointer for E-Compass info service */
  ble_char_pointer = &ble_event_counter;
  memset(ble_char_pointer, 0, sizeof(ble_char_object_t));
  ble_char_pointer->attr_mod_request_cb = attr_mod_request_event_counter;
...
/**
  ...
  */
ble_status_t ble_event_counter_update(uint32_t event_counter)
{
  ble_status_t ret;
  uint8_t buff[2 + 4];

  STORE_LE_16(buff, (HAL_GetTick() / 10U));
  STORE_LE_32(buff + 2, event_counter);

  ret = ACI_GATT_UPDATE_CHAR_VALUE(&ble_event_counter, 0, 2 + 4, buff);

  if (ret != (ble_status_t)BLE_STATUS_SUCCESS)
  {
    if (ble_std_err_service == BLE_SERV_ENABLE)
    {
      bytes_to_write = (uint8_t)sprintf((char *)buffer_to_write, "Error Updating Event Counter Char\n");
      std_err_update(buffer_to_write, bytes_to_write);
    }
    else
    {
      BLE_MANAGER_PRINTF("Error Updating Event Counter Char\r\n");
    }
  }
  return ret;
}
...
/**
  ...
  * @retval None
  */
static void attr_mod_request_event_counter(void *void_char_pointer, uint16_t attr_handle, uint16_t offset,
                                           uint8_t data_length, uint8_t *att_data)
{
  if (att_data[0] == 01U)
  {
    notify_event_event_counter(BLE_NOTIFY_SUB);
  }
  else if (att_data[0] == 0U)
  {
    notify_event_event_counter(BLE_NOTIFY_UNSUB);
  }
...
__weak void notify_event_event_counter(ble_notify_event_t event)
{
  ...
  {
    BLE_MANAGER_PRINTF("\r\nNotify event counter function not defined (It is a weak function)\r\n");
  }

  /* NOTE: This function Should not be modified, when the callback is needed,
           the notify_event_event_counter could be implemented in the user file
   */
}
