...
static ble_char_object_t ble_char_piano;

/************************************************************
  * Callback function prototype to manage the notify events *
  ***********************************************************/
__weak void notify_event_piano(ble_notify_event_t event);

/******************************************************************
  * Callback function prototype to manage the write request events *
  *****************************************************************/
__weak void write_request_piano_function(uint8_t *att_data, uint8_t data_length);

/* Private functions prototype -----------------------------------------------*/
static void attr_mod_request_piano(void *ble_char_pointer, uint16_t attr_handle, uint16_t offset, uint8_t data_length,
                                   uint8_t *att_data);
static void write_request_piano(void *ble_char_pointer, uint16_t handle, uint16_t offset, uint8_t data_length,
                                uint8_t *att_data);
...
/**
  ...
  * @param  None
  * @retval ble_char_object_t* ble_char_pointer: Data structure pointer for Piano info service
  */
ble_char_object_t *ble_init_piano_service(void)
{
  /* Data structure pointer for BLE service */
  ble_char_object_t *ble_char_pointer;

  /* Init data structure pointer for Piano info service */
  ble_char_pointer = &ble_char_piano;
  memset(ble_char_pointer, 0, sizeof(ble_char_object_t));
  ble_char_pointer->attr_mod_request_cb = attr_mod_request_piano;
  ble_char_pointer->write_request_cb = write_request_piano;
  COPY_PIANO_CHAR_UUID((ble_char_pointer->uuid));
  ble_char_pointer->char_uuid_type = UUID_TYPE_128;
  ble_char_pointer->char_value_length = 2; /* Command + Note Number */
...
/**
  ...
  */
ble_status_t ble_piano_send_buffer(uint8_t *buffer, uint32_t len)
{
  ble_status_t ret;
  ret = ACI_GATT_UPDATE_CHAR_VALUE(&ble_char_piano, 0, (uint8_t)len, buffer);
  return ret;
}
...
/**
  ...
  * @retval None
  */
static void attr_mod_request_piano(void *void_char_pointer, uint16_t attr_handle, uint16_t offset, uint8_t data_length,
                                   uint8_t *att_data)
{
  if (att_data[0] == 01U)
  {
    notify_event_piano(BLE_NOTIFY_SUB);
  }
  else if (att_data[0] == 0U)
  {
    notify_event_piano(BLE_NOTIFY_UNSUB);
  }
...
static void write_request_piano(void *ble_char_pointer, uint16_t handle, uint16_t offset, uint8_t data_length,
                                uint8_t *att_data)
{
  write_request_piano_function(att_data, data_length);
}
...
__weak void notify_event_piano(ble_notify_event_t event)
{
  ...
  {
    BLE_MANAGER_PRINTF("\r\nNotify piano function not defined (It is a weak function)\r\n");
  }

  /* NOTE: This function Should not be modified, when the callback is needed,
           the notify_event_piano could be implemented in the user file
   */
}
...
/********************************************************
  ...
  *******************************************************/
/**
  * @brief  Callback Function for piano write request.
...
__weak void write_request_piano_function(uint8_t *att_data, uint8_t data_length)
{
  ...
  BLE_MANAGER_PRINTF("\r\nWrite request piano function not defined (It is a weak function)\r\n");

  /* NOTE: This function Should not be modified, when the callback is needed,
           the write_request_piano_function could be implemented in the user file
   */
}
