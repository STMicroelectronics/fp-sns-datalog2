/**
  ******************************************************************************
  * @file    ble_medical_signal_24_bit.c
...
/* Private define ------------------------------------------------------------*/
#define COPY_MEDICAL_SIGNAL_24BIT_CHAR_UUID(uuid_struct) COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x27,0x00,\
                                                                       0x02,0x11,0xe1,0xac,0x36,0x00,0x02,0xa5,\
                                                                       0xd5,0xc5,0x1b)

/* Private variables ---------------------------------------------------------*/
/* Data structure pointer for Medical Signal 24 Bits service */
static ble_char_object_t ble_medical_signal_24_bit;

static uint16_t medical_signal_24_bit_max_char_length = DEFAULT_MAX_MEDICAL_SIGNAL_24BIT_CHAR_LEN;

/* Private functions ---------------------------------------------------------*/
static void attr_mod_request_medical_signal_24_bit(void *ble_char_pointer, uint16_t attr_handle, uint16_t offset,
...
extern ble_char_object_t *ble_init_medical_signal_24_bit_service(uint16_t max_lenght)
{
  ...
  ble_char_pointer = &ble_medical_signal_24_bit;
  memset(ble_char_pointer, 0, sizeof(ble_char_object_t));
  ble_char_pointer->attr_mod_request_cb = attr_mod_request_medical_signal_24_bit;
  COPY_MEDICAL_SIGNAL_24BIT_CHAR_UUID((ble_char_pointer->uuid));
  ble_char_pointer->char_uuid_type = UUID_TYPE_128;
  ble_char_pointer->char_uuid_type = max_lenght;
  ...
  medical_signal_24_bit_max_char_length = max_lenght;
...
/**
  ...
  * @param  int32_t *sample_data  Values array
  * @retval ble_status_t   Status
  */
ble_status_t ble_medical_signal_24_bit_update(ble_medical_signal_24_bit_t data_type_id, uint32_t time_stamp,
                                              ...
                                              int32_t *sample_data)
{
  ...
  int32_t counter;
  int32_t total_size = sample_data_size * 3 /*3 == 24/8 */ + 4 /* TimeStamp */ + 1 /* data_type_id */;

  if (total_size > medical_signal_24_bit_max_char_length)
  {
    BLE_MANAGER_PRINTF("BLE MedicalSig24Bits Error ... Too many Bytes %d>%d\r\n", total_size,
                       medical_signal_24_bit_max_char_length);
  ...
  {
    STORE_LE_24(buff + 5 + (counter * 3), sample_data[counter]);
  }

  ret = ACI_GATT_UPDATE_CHAR_VALUE(&ble_medical_signal_24_bit, 0, total_size, buff);

  if (ret != (ble_status_t)BLE_STATUS_SUCCESS)
  {
    if (ble_std_err_service == BLE_SERV_ENABLE)
    {
      bytes_to_write = (uint8_t)sprintf((char *)buffer_to_write, "Error Updating MedicalSig24Bis Char\n");
...
/**
  ...
  */
uint16_t ble_medical_signal_24_bit_get_max_char_length(void)
{
  return medical_signal_24_bit_max_char_length;
}
...
/**
  ...
  * @retval none
  */
void ble_medical_signal_24_bit_set_max_char_length(uint16_t max_char_length)
{
  medical_signal_24_bit_max_char_length = max_char_length;
}
...
/**
  ...
  * @retval None
  */
static void attr_mod_request_medical_signal_24_bit(void *void_char_pointer, uint16_t attr_handle, uint16_t offset,
                                                   uint8_t data_length, uint8_t *att_data)
{
  if (att_data[0] == 01U)
  {
    notify_event_medical_signal_24_bit(BLE_NOTIFY_SUB);
  }
  else if (att_data[0] == 0U)
  {
    notify_event_medical_signal_24_bit(BLE_NOTIFY_UNSUB);
  }
...
/**
  ...
  * @retval None
  */
void notify_event_medical_signal_24_bit(ble_notify_event_t event)
{
  /* Prevent unused argument(s) compilation warning */
  BLE_UNUSED(event);

  BLE_MANAGER_PRINTF("\r\nNotify medical signal 24 bit function not defined (It is a weak function)\r\n");
  /* NOTE: This function Should not be modified, when the callback is needed,
           the notify_event_medical_signal_24_bit could be implemented in the user file
   */
}