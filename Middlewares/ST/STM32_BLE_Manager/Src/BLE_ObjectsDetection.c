...
/* Private functions ---------------------------------------------------------*/
static void AttrMod_Request_ObjectsDetection(void *BleCharPointer,uint16_t attr_handle, uint16_t Offset, uint8_t data_length, uint8_t *att_data);
static void Write_Request_ObjectsDetection(void *VoidCharPointer,uint16_t handle, uint16_t Offset, uint8_t data_length, uint8_t *att_data);
...
BleCharTypeDef* BLE_InitObjectsDetectionService(void)
{
  ...
  BleCharPointer->Char_Properties = ((uint8_t)CHAR_PROP_NOTIFY) | ((uint8_t)CHAR_PROP_WRITE_WITHOUT_RESP) | ((uint8_t)CHAR_PROP_WRITE);
