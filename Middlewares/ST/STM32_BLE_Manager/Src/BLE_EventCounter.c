...
/* Private define ------------------------------------------------------------*/
#define COPY_EVENT_COUNTER_CHAR_UUID(uuid_struct) COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x1D,0x00,0x02,0x11,0xe1,0xac,0x36,0x00,0x02,0xa5,0xd5,0xc5,0x1b)
...
/* Private functions ---------------------------------------------------------*/
static void AttrMod_Request_EventCounter(void *BleCharPointer,uint16_t attr_handle, uint16_t Offset, uint8_t data_length, uint8_t *att_data);
 ...
 * @retval None
 */
static void AttrMod_Request_EventCounter(void *VoidCharPointer, uint16_t attr_handle, uint16_t Offset, uint8_t data_length, uint8_t *att_data)
{
 ...
 if(BLE_StdTerm_Service==BLE_SERV_ENABLE) {
   BytesToWrite = (uint8_t) sprintf((char *)BufferToWrite,"--->EventCount=%s\n", (att_data[0] == 01U) ? " ON" : " OFF");
