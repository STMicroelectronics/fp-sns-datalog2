...
static BleCharTypeDef BlePedometerAlgorithm;

/* Private functions ---------------------------------------------------------*/
static void AttrMod_Request_PedometerAlgorithm(void *BleCharPointer, uint16_t attr_handle, uint16_t Offset,
...
/**
  ...
  * @param  None
  * @retval BleCharTypeDef* BleCharPointer: Data structure pointer for Pedometer Algorithm service
  */
BleCharTypeDef *BLE_InitPedometerAlgorithmService(void)
{
  /* Data structure pointer for BLE service */
  BleCharTypeDef *BleCharPointer;

  /* Init data structure pointer for Pedometer Algorithm info service */
  BleCharPointer = &BlePedometerAlgorithm;
  memset(BleCharPointer, 0, sizeof(BleCharTypeDef));
  BleCharPointer->AttrMod_Request_CB = AttrMod_Request_PedometerAlgorithm;
  BleCharPointer->Read_Request_CB = Read_Request_PedometerAlgorithm;
  COPY_PEDOMETER_ALGORITHM_CHAR_UUID((BleCharPointer->uuid));
  BleCharPointer->Char_UUID_Type = UUID_TYPE_128;
  BleCharPointer->Char_Value_Length = 2 + 4 + 2; /* 2 byte timestamp, 4 byte step, 2 byte cadence */
  ...
  {
    BLE_MANAGER_PRINTF("Warning: Read request Pedometer Algorithm function not defined\r\n");
  }
...
/**
  ...
  */
tBleStatus BLE_PedometerAlgorithmUpdate(BLE_PM_output_t *PedometerAlgorithmData)
{
  tBleStatus ret;
  uint8_t buff[2 + 4 + 2];
  uint16_t Cadence = (uint16_t) PedometerAlgorithmData->Cadence;

  STORE_LE_16(buff, (HAL_GetTick() >> 3));
  STORE_LE_32(buff + 2, PedometerAlgorithmData->Nsteps);
  STORE_LE_16(buff + 6, Cadence);

  ret = ACI_GATT_UPDATE_CHAR_VALUE(&BlePedometerAlgorithm, 0, 2 + 4 + 2, buff);

  if (ret != (tBleStatus)BLE_STATUS_SUCCESS)
  {
    if (BLE_StdErr_Service == BLE_SERV_ENABLE)
    {
      BytesToWrite = (uint8_t)sprintf((char *)BufferToWrite, "Error Updating Pedometer Algorithm Char\n");
...
/**
  ...
  *         With this function it's possible to understand if Pedometer Algorithm is subscribed or not to the one service
  ...
  * @retval None
  */
static void AttrMod_Request_PedometerAlgorithm(void *VoidCharPointer, uint16_t attr_handle, uint16_t Offset,
                                               uint8_t data_length, uint8_t *att_data)
{
...
static void Read_Request_PedometerAlgorithm(void *VoidCharPointer, uint16_t handle)
{
  if (CustomReadRequestPedometerAlgorithm != NULL)
  {
    BLE_PM_output_t PedometerAlgorithmData;
    CustomReadRequestPedometerAlgorithm(&PedometerAlgorithmData);
    BLE_PedometerAlgorithmUpdate(&PedometerAlgorithmData);
  }
  else
  {
                                            ...
                                            uint8_t Data[])
{
  ...
  if (CustomReadRequestPedometerAlgorithm != NULL)
  {
    ...
    Cadence = (uint16_t) PedometerAlgorithmData.Cadence;
    STORE_LE_16(buff, (HAL_GetTick() >> 3));
    STORE_LE_32(buff + 2, PedometerAlgorithmData.Nsteps);
    STORE_LE_16(buff + 6, Cadence);
