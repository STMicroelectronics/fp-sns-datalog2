...
typedef PACKED(struct) _hci_spi_pckt{
  uint8_t type;
...
typedef PACKED(struct) _hci_event_hdr{
  uint8_t evt;
  uint8_t plen;
}  hci_event_hdr;
...
#define HCI_COMMAND_EXT_HDR_SIZE 4
...
typedef PACKED(struct) _hci_event_ext_hdr{
  uint8_t  evt;
  uint16_t plen;
}  hci_event_ext_hdr;
#define HCI_EVENT_EXT_HDR_SIZE 	3

typedef PACKED(struct) _hci_vendor_hdr{
  uint8_t  opcode;
  uint16_t plen;
} hci_vendor_hdr;
#define HCI_VENDOR_HDR_SIZE 3
...
typedef PACKED(struct) _disconnect_cp{
  uint16_t handle;
  uint8_t  reason;
}  disconnect_cp;
...
typedef PACKED(struct) _read_transmit_power_level_cp{
  uint16_t handle;
  uint8_t  type;
}  read_transmit_power_level_cp;
#define READ_TRANSMIT_POWER_LEVEL_CP_SIZE 3
typedef PACKED(struct) _read_transmit_power_level_rp{
  uint8_t  status;
  uint16_t handle;
  int8_t   level;
...
typedef PACKED(struct) _read_local_version_rp{
  uint8_t  status;
  uint8_t  hci_version;
...
typedef PACKED(struct) _read_rssi_cp{
  uint16_t handle;
}  read_rssi_cp;
#define READ_RSSI_CP_SIZE 2
typedef PACKED(struct) _read_rssi_rp{
  uint8_t  status;
  uint16_t handle;
  int8_t   rssi;
}  read_rssi_rp;
...
typedef PACKED(struct) _le_set_event_mask_cp{
  uint8_t mask[8];
}  le_set_event_mask_cp;
#define LE_SET_EVENT_MASK_CP_SIZE 8

#define OCF_LE_READ_BUFFER_SIZE			0x0002
typedef PACKED(struct) _le_read_buffer_size_rp{
  uint8_t  status;
  uint16_t pkt_len;
  uint8_t  max_pkt;
...
typedef PACKED(struct) _le_read_local_supported_features_rp{
  uint8_t status;
  uint8_t features[8];
...
typedef PACKED(struct) _le_set_adv_parameters_cp{
...
typedef PACKED(struct) _le_read_adv_channel_tx_power_rp{
  uint8_t status;
  int8_t  level;
...
typedef PACKED(struct) _le_set_adv_data_cp{
  uint8_t length;
  uint8_t data[31];
}  le_set_adv_data_cp;
...
typedef PACKED(struct) _le_set_scan_response_data_cp{
  uint8_t length;
  uint8_t data[31];
...
typedef PACKED(struct) _le_set_advertise_enable_cp{
  uint8_t enable;
}  le_set_advertise_enable_cp;
#define LE_SET_ADVERTISE_ENABLE_CP_SIZE 1

#define OCF_LE_SET_SCAN_PARAMETERS		0x000B
typedef PACKED(struct) _le_set_scan_parameters_cp{
  uint8_t  type;
  uint16_t interval;
  uint16_t window;
  uint8_t  own_bdaddr_type;
...
typedef PACKED(struct) _le_set_scan_enable_cp{
  uint8_t enable;
  uint8_t filter_dup;
...
typedef PACKED(struct) _le_create_connection_cp{
  uint16_t interval;
  uint16_t window;
...
typedef PACKED(struct) _le_read_white_list_size_rp{
  uint8_t status;
  uint8_t size;
...
typedef PACKED(struct) _le_connection_update_cp{
  uint16_t handle;
  uint16_t min_interval;
  uint16_t max_interval;
  uint16_t latency;
...
typedef PACKED(struct) _le_set_host_channel_classification_cp{
  uint8_t map[5];
...
typedef PACKED(struct) _le_read_channel_map_cp{
  uint16_t handle;
}  le_read_channel_map_cp;
#define LE_READ_CHANNEL_MAP_CP_SIZE 2

typedef PACKED(struct) _le_read_channel_map_rp{
  uint8_t  status;
  uint16_t handle;
  uint8_t  map[5];
...
typedef PACKED(struct) _le_rand_rp{
  uint8_t status;
  uint8_t random[8];
}  le_rand_rp;
...
typedef PACKED(struct) _le_start_encryption_cp{
  uint16_t handle;
  uint8_t  random[8];
...
typedef PACKED(struct) _le_ltk_reply_cp{
  uint16_t handle;
  uint8_t  key[16];
}  le_ltk_reply_cp;
#define LE_LTK_REPLY_CP_SIZE 18

typedef PACKED(struct) _le_ltk_reply_rp{
  uint8_t  status;
  uint16_t handle;
}  le_ltk_reply_rp;
...
typedef PACKED(struct) _le_ltk_neg_reply_cp{
  uint16_t handle;
}  le_ltk_neg_reply_cp;
#define LE_LTK_NEG_REPLY_CP_SIZE 2

typedef PACKED(struct) _le_ltk_neg_reply_rp{
  uint8_t  status;
  uint16_t handle;
}  le_ltk_neg_reply_rp;
...
typedef PACKED(struct) _le_read_supported_states_rp{
  uint8_t status;
  uint8_t states[8];
...
typedef PACKED(struct) _le_receiver_test_cp{
  uint8_t frequency;
}  le_receiver_test_cp;
#define LE_RECEIVER_TEST_CP_SIZE 1

#define OCF_LE_TRANSMITTER_TEST			0x001E
typedef PACKED(struct) _le_transmitter_test_cp{
  uint8_t frequency;
  uint8_t length;
  uint8_t payload;
...
typedef PACKED(struct) _le_test_end_rp{
  uint8_t  status;
  uint16_t num_pkts;
}  le_test_end_rp;
...
typedef PACKED(struct) _evt_conn_complete{
  uint8_t  status;
  uint16_t handle;
  tBDAddr  bdaddr;
  uint8_t  link_type;
...
typedef PACKED(struct) _evt_disconn_complete{
  uint8_t  status;
  uint16_t handle;
  uint8_t  reason;
...
typedef PACKED(struct) _evt_encrypt_change{
  uint8_t  status;
  uint16_t handle;
  uint8_t  encrypt;
}  evt_encrypt_change;
...
typedef PACKED(struct) _evt_cmd_complete{
  uint8_t  ncmd;
  uint16_t opcode;
}  evt_cmd_complete;
...
typedef PACKED(struct) _evt_cmd_status{
  uint8_t  status;
  uint8_t  ncmd;
  uint16_t opcode;
}  evt_cmd_status;
...
typedef PACKED(struct) _evt_hardware_error{
  uint8_t code;
}  evt_hardware_error;
#define EVT_HARDWARE_ERROR_SIZE 1

#define EVT_NUM_COMP_PKTS		0x13
typedef PACKED(struct) _evt_num_comp_pkts{
  uint8_t num_hndl;
...
typedef PACKED(struct) _evt_num_comp_pkts_param{
  uint16_t hndl;
...
typedef PACKED(struct) _evt_data_buffer_overflow{
  uint8_t link_type;
}  evt_data_buffer_overflow;
#define EVT_DATA_BUFFER_OVERFLOW_SIZE 1

#define EVT_ENCRYPTION_KEY_REFRESH_COMPLETE	0x30
typedef PACKED(struct) _evt_encryption_key_refresh_complete{
  uint8_t  status;
  uint16_t handle;
...
typedef PACKED(struct) _evt_le_connection_complete{
  uint8_t  status;
  uint16_t handle;
  uint8_t  role;
  uint8_t  peer_bdaddr_type;
...
typedef PACKED(struct) _le_advertising_info{
  uint8_t evt_type;
  uint8_t bdaddr_type;
  tBDAddr bdaddr;
  uint8_t data_length;
...
typedef PACKED(struct) _evt_le_connection_update_complete{
  uint8_t  status;
  uint16_t handle;
  uint16_t interval;
  uint16_t latency;
...
typedef PACKED(struct) _evt_le_read_remote_used_features_complete{
  uint8_t  status;
  uint16_t handle;
  uint8_t  features[8];
...
typedef PACKED(struct) _evt_le_long_term_key_request{
  uint16_t handle;
  uint8_t  random[8];
  uint16_t ediv;
