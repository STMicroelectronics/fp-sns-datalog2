...
#define INIT_TASK_CFG_QUEUE_ITEM_SIZE          (uint32_t)(sizeof(SysEvent))
#ifndef INIT_TASK_CFG_QUEUE_LENGTH
#define INIT_TASK_CFG_QUEUE_LENGTH             16U
#endif
...
#define INIT_IS_KIND_OF_AMTEX(pxTask)           ((pxTask)->m_xStatus.nReserved == 1U)
...
/**
  ...
  */
static uint16_t InitTaskDoEnterPowerModeForPMClass(ApplicationContext *pxContext, EPMClass ePowerModeClass,
                                                   const EPowerMode eActivePowerMode, const EPowerMode eNewPowerMode);
...
sys_error_code_t SysInit(boolean_t bEnableBootIF)
{
  ...
  if (xTaskCreate(InitTaskRun, "INIT", INIT_TASK_CFG_STACK_SIZE, NULL, INIT_TASK_CFG_PRIORITY,
                  &s_xTheSystem.m_xInitTask) != pdPASS)
  {
...
APIVersion SysGetAPIVersion(void)
{
  union
  {
    APIVersion xVersion;
...
static void InitTaskRun(ULONG thread_input)
{
  ...
  if (TX_SUCCESS != tx_byte_allocate(&s_xTheSystem.m_xSysMemPool, (VOID **)&pcMemory,
                                     INIT_TASK_CFG_QUEUE_ITEM_SIZE * INIT_TASK_CFG_QUEUE_LENGTH, TX_NO_WAIT))
  {
    sys_error_handler();
  }
  ...
  (void)SysOnStartApplication(&xContext);
  (void)IAEDOnStartApplication(s_xTheSystem.m_pxAppErrorDelegate, &xContext);
  ...
  while ((pxTask != NULL) && (!SYS_IS_ERROR_CODE(xRes)))
  {
    if (pxTask->m_xStatus.nAutoStart == 1U)
    {
      (void)tx_thread_resume(&pxTask->m_xTaskHandle);
    }
    pxTask = ACGetNextTask(&xContext, pxTask);
  }
  ...
  for (;;)
  {
    if (TX_SUCCESS == tx_queue_receive(&s_xTheSystem.m_xSysQueue, &xEvent, TX_WAIT_FOREVER))
    {
      ...
      if (SYS_IS_ERROR_EVENT(xEvent))
      {
        (void)IAEDProcessEvent(s_xTheSystem.m_pxAppErrorDelegate, &xContext, xEvent);
        ...
        if (IapmhIsLowPowerMode(s_xTheSystem.m_pxAppPowerModeHelper, eActivePowerMode))
        {
          ...
          if (!IAEDIsLastErrorPending(s_xTheSystem.m_pxAppErrorDelegate))
          {
            ...
            (void)IapmhDidEnterPowerMode(s_xTheSystem.m_pxAppPowerModeHelper, eActivePowerMode);
          }
        }
      }
      else
      {
        ...
        {
          (void)IapmhCheckPowerModeTransaction(s_xTheSystem.m_pxAppPowerModeHelper, eActivePowerMode, ePowerMode);
          ...
          (void)IapmhDidEnterPowerMode(s_xTheSystem.m_pxAppPowerModeHelper, ePowerMode);
      ...
      if (eTaskPMClass == ePowerModeClass)
      {
        ...
        if (pTask->m_xStatus.nPowerModeSwitchDone == 0U)
        {
          if ((pTask->m_xStatus.nDelayPowerModeSwitch == 0U))
          {
            ...
            (void)AMTDoEnterPowerMode(pTask, eActivePowerMode, eNewPowerMode);
          ...
          else
          {
            ...
            if (pTask->m_xStatus.nReserved == 1U)
            {
              ...
              (void)AMTExForceExecuteStep((AManagedTaskEx *)pTask, eActivePowerMode);
            }
