#!/bin/bash
# Setup script for hsdatalog under Linux on Raspberry Pi 64-bit
# NOTE: unplug the board before running the script
echo "NOTE: unplug the board before running the script"

# Copy the shared library
if [ -f ../lib/libhs_datalog/raspberryPi_64bit/libhs_datalog_v2.so ]; then
...
if [ -f ../bin_raspberryPi_64bit/cli_example ]; then
    chmod 755 ../bin_raspberryPi_64bit/cli_example
else
