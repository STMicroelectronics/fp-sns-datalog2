...
#define ST_HS_DATALOG_DISCONNECTED -2

void hs_datalog_start_hotplug_monitor();
void hs_datalog_stop_hotplug_monitor();
 ...
 * @return ST_HS_DATALOG_OK if no error occurred, ST_HS_DATALOG_ERROR or ST_HS_DATALOG_DISCONNECTED
 */
...
/**
  * @deprecated This function is deprecated. Use hs_datalog_load_ucf_file_to_mlc instead.
 ...
 * @param dID Id of the device.
 * @param comp_name Id of the sensor Component.
 * @param ucf_file_path Path to the UCF file.
 * @param response PnPL Command response
 * 
 * @return ST_HS_DATALOG_OK if no error occurred, ST_HS_DATALOG_ERROR
 * otherwise
 */
int hs_datalog_load_ucf_file_to_mlc(int dID, char* comp_name, char* ucf_file_path, char **response);
