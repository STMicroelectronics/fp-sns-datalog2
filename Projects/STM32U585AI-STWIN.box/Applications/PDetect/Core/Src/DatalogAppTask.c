...
sys_error_code_t DatalogAppTask_OnNewDataReady_vtbl(IEventListener *_this, const DataEvent_t *p_evt)
{
  ...
  if (nPMState == E_POWER_MODE_SENSORS_ACTIVE)
  {
    ...
    if (p_obj->sensorContext[sId].old_time_stamp == -1.0f)
    {
      ...
      else if (sensor_status.isensor_class == ISENSOR_CLASS_LIGHT)
      {
        ODR = (1.0f / (float_t)sensor_status.type.light.intermeasurement_time);
      }
      else if (sensor_status.isensor_class == ISENSOR_CLASS_PRESENCE)
      {
        ODR = sensor_status.type.presence.data_frequency;
      }
      else if (sensor_status.isensor_class == ISENSOR_CLASS_RANGING)
      {
        ODR = sensor_status.type.ranging.profile_config.frequency;
      }
      else
      {
        SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    ...
    while (samplesToSend > 0)
    {
      ...
      else
      {
        ...
        {
          measuredODR = (1.0f / (float_t)sensor_status.type.light.measured_intermeasurement_time);
        }
        else if (sensor_status.isensor_class == ISENSOR_CLASS_PRESENCE)
        {
          measuredODR = sensor_status.type.presence.measured_data_frequency;
        }
        else if (sensor_status.isensor_class == ISENSOR_CLASS_RANGING)
        {
          measuredODR = sensor_status.type.ranging.profile_config.frequency;
        }
        else
        {
          SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
