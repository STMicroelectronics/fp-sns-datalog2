...
{
  float_t channel1_gain = vd6283tx_3_als_model.sensor_status->type.light.gain[0];
...
{
  float_t channel2_gain = vd6283tx_3_als_model.sensor_status->type.light.gain[1];
...
{
  float_t channel3_gain = vd6283tx_3_als_model.sensor_status->type.light.gain[2];
...
{
  float_t channel4_gain = vd6283tx_3_als_model.sensor_status->type.light.gain[3];
...
{
  float_t channel5_gain = vd6283tx_3_als_model.sensor_status->type.light.gain[4];
...
{
  float_t channel6_gain = vd6283tx_3_als_model.sensor_status->type.light.gain[5];
...
uint8_t vd6283tx_3_als_get_ioffset(float_t *value)
{
...
uint8_t vd6283tx_3_als_set_exposure_time(int32_t value, char **response_message)
{
  ...
  if (value >= min_v && value <= max_v)
  {
    ...
    if (ret == SYS_NO_ERROR_CODE)
    {
      ...
      {
        spts = (1000.0f / (float_t)(vd6283tx_3_als_model.sensor_status->type.light.intermeasurement_time));
      }
      else
      {
        spts = (1000.0f / (float_t)(vd6283tx_3_als_model.sensor_status->type.light.exposure_time / 1000 + 6));
      }
