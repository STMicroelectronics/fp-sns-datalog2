...
uint8_t Imp34dt05_Mic_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  int32_t temp_i = 0;
  imp34dt05_mic_get_odr(&temp_i);
  uint8_t enum_id = 0;
  if (temp_i == imp34dt05_mic_odr_hz16000)
  {
    enum_id = 0;
  }
  else if (temp_i == imp34dt05_mic_odr_hz32000)
  {
    enum_id = 1;
  }
  else if (temp_i == imp34dt05_mic_odr_hz48000)
  {
    enum_id = 2;
  }
  ...
  imp34dt05_mic_get_resolution(&temp_i);
  enum_id = 0;
  if (temp_i == imp34dt05_mic_resolution_bit16)
  {
    enum_id = 0;
  }
                                           ...
                                           uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.sensor_annotation"))
  {
    ...
    if (ret == 0)
    {
      json_object_dotset_string(respJSONObject, "imp34dt05_mic.sensor_annotation.value", sensor_annotation);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "imp34dt05_mic.sensor_annotation.value", "PNPL_SET_ERROR");
    }
  }
  json_value_free(tempJSON);
  if (pretty == 1)
