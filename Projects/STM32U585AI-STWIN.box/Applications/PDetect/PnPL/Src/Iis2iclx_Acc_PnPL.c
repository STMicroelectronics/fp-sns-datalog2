                                          ...
                                          uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "iis2iclx_acc.sensor_annotation"))
  {
    ...
    if (ret == 0)
    {
      json_object_dotset_string(respJSONObject, "iis2iclx_acc.sensor_annotation.value", sensor_annotation);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "iis2iclx_acc.sensor_annotation.value", "PNPL_SET_ERROR");
    }
  }
  json_value_free(tempJSON);
  if (pretty == 1)
