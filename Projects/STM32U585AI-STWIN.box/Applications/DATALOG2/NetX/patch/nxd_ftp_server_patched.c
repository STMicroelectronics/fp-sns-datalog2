  ...
  for (i = 0; i < NX_FTP_MAX_CLIENTS; i ++)
  {
    ...
    if (client_request_ptr -> nx_ftp_client_request_data_socket.nx_tcp_socket_id)
    {
      ...
      if (client_request_ptr -> nx_ftp_client_request_passive_transfer_enabled == NX_TRUE)
      {
        ...
        nx_tcp_server_socket_unlisten(ftp_server_ptr -> nx_ftp_server_ip_ptr,
                                      client_request_ptr -> nx_ftp_client_request_data_socket.nx_tcp_socket_port);
      }
      else
      {
...
/*                                                                        */
/**************************************************************************/
VOID  _nx_ftp_server_directory_response(NX_TCP_SOCKET *socket, NX_PACKET *packet_ptr, CHAR *reply_code, CHAR *message,
                                        CHAR *directory)
{
...
/*    nx_tcp_socket_transmit_configure      Configure data transfer socket*/
          ...
          if (status == FX_SUCCESS)
          {
            ...
            if (client_req_ptr -> nx_ftp_client_request_passive_transfer_enabled == NX_TRUE)
            {
              ...
              if (status)
              {
                ...
                nx_tcp_server_socket_unlisten(ftp_server_ptr -> nx_ftp_server_ip_ptr,
                                              client_req_ptr -> nx_ftp_client_request_data_socket.nx_tcp_socket_port);
  ...
  for (i = 0; i < NX_FTP_MAX_CLIENTS; i++)
  {
    ...
    if (client_req_ptr -> nx_ftp_client_request_activity_timeout)
    {
      ...
      if (client_req_ptr -> nx_ftp_client_request_activity_timeout == 0)
      {
        ...
        nx_tcp_server_socket_relisten(ftp_server_ptr -> nx_ftp_server_ip_ptr, NX_FTP_SERVER_CONTROL_PORT,
                                      &(client_req_ptr -> nx_ftp_client_request_control_socket));
        /* notify the application cb about timed out client connection  */
        printf("ERROR FTP srv Client activity timeout\n\r");
#ifndef NX_DISABLE_IPV4
        /* Does this server have an IPv4 logout function? */
        if (ftp_server_ptr -> nx_ftp_logout_ipv4)
        {
          /* Call the logout which takes IPv4 address input. */
          (ftp_server_ptr -> nx_ftp_logout_ipv4)(ftp_server_ptr, IPv4IP, port,
                                                 client_req_ptr -> nx_ftp_client_request_username,
                                                 client_req_ptr -> nx_ftp_client_request_password, NX_NULL);
        }
#endif /* NX_DISABLE_IPV4 */
        ...
        {
          /* Call the 'duo' logout function which takes IPv6 or IPv4 IP addresses. */
          (ftp_server_ptr -> nx_ftp_logout)(ftp_server_ptr, &IPv6IP, port,
                                            client_req_ptr -> nx_ftp_client_request_username,
                                            client_req_ptr -> nx_ftp_client_request_password, NX_NULL);
        }
...
UINT _nx_ftp_packet_allocate(NX_PACKET_POOL *pool_ptr, NX_FTP_CLIENT_REQUEST *client_request_ptr,
                             NX_PACKET **packet_ptr, UINT packet_type, UINT wait_option)
{
...
UINT _nx_ftp_utility_parse_IPv6_address(CHAR *buffer_ptr, UINT buffer_length, NXD_ADDRESS *ipduo_address)
{
  ...
  while (j < buffer_length)
  {
    ...
    else  /* No hex, not a separator or part of the EPRT command. */
    {
...
/*                                                                        */
/**************************************************************************/
UINT  _nx_ftp_server_block_header_send(NX_PACKET_POOL *pool_ptr, NX_FTP_CLIENT_REQUEST *client_request_ptr,
                                       ULONG block_size)
{
