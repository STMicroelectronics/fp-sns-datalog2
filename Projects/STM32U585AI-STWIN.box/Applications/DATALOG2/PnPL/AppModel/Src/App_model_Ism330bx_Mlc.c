...
static SensorModel_t ism330bx_mlc_model;
extern AppModel_t app_model;

uint8_t ism330bx_mlc_comp_init(void)
{
  ism330bx_mlc_model.comp_name = ism330bx_mlc_get_key();

  SQuery_t querySM;
  SQInit(&querySM, SMGetSensorManager());
  uint16_t id = SQNextByNameAndType(&querySM, "ism330bx", COM_TYPE_MLC);
  ism330bx_mlc_model.id = id;
  ism330bx_mlc_set_st_ble_stream__id(id, NULL);

  ism330bx_mlc_model.sensor_status = SMSensorGetStatusPointer(id);
  ...
  ism330bx_mlc_set_sensor_annotation("[EXTERN]\0", NULL);
#if (HSD_USE_DUMMY_DATA == 1)
  ism330bx_mlc_model.stream_params.spts = 0;
#else
  ism330bx_mlc_model.stream_params.spts = 1;
#endif
  ...
  __sc_set_ble_stream_params(ism330bx_mlc_model.id);
...
{
  *value = ism330bx_mlc_model.sensor_status->is_active;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
{
  *value = ism330bx_mlc_model.stream_params.spts;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
{
  *value = app_model.ism330bx_mlc_ucf_valid;
  return PNPL_NO_ERROR_CODE;
}
...
uint8_t ism330bx_mlc_get_ioffset(float_t *value)
{
  *value = ism330bx_mlc_model.stream_params.ioffset;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
{
  *value = ism330bx_mlc_model.stream_params.usb_dps;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
{
  *value = ism330bx_mlc_model.stream_params.sd_dps;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
{
  *value = ism330bx_mlc_model.annotation;
  return PNPL_NO_ERROR_CODE;
}

uint8_t ism330bx_mlc_get_sensor_category(int32_t *value)
{
  *value = ism330bx_mlc_model.sensor_status->isensor_class;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
uint8_t ism330bx_mlc_get_st_ble_stream__mlc_multiply_factor(float_t *value)
{
...
{
  *value = ism330bx_mlc_model.stream_params.stream_id;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
{
  *value = ism330bx_mlc_model.stream_params.usb_ep;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}
...
uint8_t ism330bx_mlc_set_enable(bool value, char **response_message)
{
  ...
  if (app_model.ism330bx_mlc_ucf_valid == true)
  {
    if (value)
    {
      ret = SMSensorEnable(ism330bx_mlc_model.id);
    }
    else
    {
      ret = SMSensorDisable(ism330bx_mlc_model.id);
    }
...
uint8_t ism330bx_mlc_set_sensor_annotation(const char *value, char **response_message)
{
  ...
  uint8_t ret = PNPL_NO_ERROR_CODE;
  strcpy(ism330bx_mlc_model.annotation, value);
  return ret;
}
...
uint8_t ism330bx_mlc_set_st_ble_stream__mlc_multiply_factor(float_t value, char **response_message)
{
  if (response_message != NULL)
  {
    *response_message = "";
...
{
  DatalogAppTask_load_ism330bx_ucf_vtbl(data, size);
  app_model.ism330bx_mlc_ucf_valid = true;
  __stream_control(true);
  __sc_set_ble_stream_params(ism330bx_mlc_model.id);
  ...
  id = SQNextByNameAndType(&query, "ism330bx", COM_TYPE_ACC);
  __sc_set_ble_stream_params(id);
  SQInit(&query, SMGetSensorManager());
  id = SQNextByNameAndType(&query, "ism330bx", COM_TYPE_GYRO);
  __sc_set_ble_stream_params(id);
  return PNPL_NO_ERROR_CODE;
}
