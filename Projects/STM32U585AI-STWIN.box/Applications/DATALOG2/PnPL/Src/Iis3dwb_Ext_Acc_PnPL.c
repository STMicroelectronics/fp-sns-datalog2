                                           ...
                                           uint8_t pretty)
{
  ...
  float temp_f = 0;
  iis3dwb_ext_acc_get_odr(&temp_f);
  uint8_t enum_id = 0;
  if (temp_f == iis3dwb_ext_acc_odr_hz26667)
  {
    enum_id = 0;
  }
  json_object_dotset_number(JSON_Status, "iis3dwb_ext_acc.odr", enum_id);
  iis3dwb_ext_acc_get_fs(&temp_f);
  enum_id = 0;
  if (temp_f == iis3dwb_ext_acc_fs_g2)
  {
    enum_id = 0;
  }
  else if (temp_f == iis3dwb_ext_acc_fs_g4)
  {
    enum_id = 1;
  }
  else if (temp_f == iis3dwb_ext_acc_fs_g8)
  {
    enum_id = 2;
  }
  else if (temp_f == iis3dwb_ext_acc_fs_g16)
  {
    enum_id = 3;
  }
...
uint8_t Iis3dwb_Ext_Acc_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.sensor_annotation"))
  {
    ...
    json_object_dotset_string(respJSONObject, "iis3dwb_ext_acc.sensor_annotation.value", ret == 0 ? sensor_annotation : "PNPL_SET_ERROR");
  }
  json_value_free(tempJSON);
  if (pretty == 1)
  {
