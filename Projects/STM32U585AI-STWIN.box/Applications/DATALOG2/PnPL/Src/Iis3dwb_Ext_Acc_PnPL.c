...
#include "Iis3dwb_Ext_Acc_PnPL_vtbl.h"
                                           ...
                                           uint8_t pretty)
{
  ...
  int32_t temp_i = 0;
  iis3dwb_ext_acc_get_odr(&temp_i);
  uint8_t enum_id = 0;
  if (temp_i == iis3dwb_ext_acc_odr_hz26667)
  {
    enum_id = 0;
  }
  json_object_dotset_number(JSON_Status, "iis3dwb_ext_acc.odr", enum_id);
  iis3dwb_ext_acc_get_fs(&temp_i);
  enum_id = 0;
  if (temp_i == iis3dwb_ext_acc_fs_g2)
  {
    enum_id = 0;
  }
  else if (temp_i == iis3dwb_ext_acc_fs_g4)
  {
    enum_id = 1;
  }
  else if (temp_i == iis3dwb_ext_acc_fs_g8)
  {
    enum_id = 2;
  }
  else if (temp_i == iis3dwb_ext_acc_fs_g16)
  {
    enum_id = 3;
  }
  json_object_dotset_number(JSON_Status, "iis3dwb_ext_acc.fs", enum_id);
  bool temp_b = 0;
                                             ...
                                             uint32_t *size, uint8_t pretty)
{
  ...
  {
    int fs = (int)json_object_dotget_number(tempJSONObject, "iis3dwb_ext_acc.fs");
    switch (fs)
    {
      case 0:
        ret = iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g2);
        break;
      case 1:
        ret = iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g4);
        break;
      case 2:
        ret = iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g8);
        break;
      case 3:
        ret = iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g16);
        break;
    }
    if (ret == 0)
    {
      json_object_dotset_number(respJSONObject, "iis3dwb_ext_acc.fs.value", fs);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "iis3dwb_ext_acc.fs.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.enable"))
  {
    ...
    ret = iis3dwb_ext_acc_set_enable(enable);
    if (ret == 0)
    {
      json_object_dotset_boolean(respJSONObject, "iis3dwb_ext_acc.enable.value", enable);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "iis3dwb_ext_acc.enable.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.samples_per_ts"))
  {
    ...
    ret = iis3dwb_ext_acc_set_samples_per_ts(samples_per_ts);
    if (ret == 0)
    {
      json_object_dotset_number(respJSONObject, "iis3dwb_ext_acc.samples_per_ts.value", samples_per_ts);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "iis3dwb_ext_acc.samples_per_ts.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.sensor_annotation"))
  {
    ...
    ret = iis3dwb_ext_acc_set_sensor_annotation(sensor_annotation);
    if (ret == 0)
    {
      json_object_dotset_string(respJSONObject, "iis3dwb_ext_acc.sensor_annotation.value", sensor_annotation);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "iis3dwb_ext_acc.sensor_annotation.value", "PNPL_SET_ERROR");
    }
  }
  json_value_free(tempJSON);
  if (pretty == 1)
