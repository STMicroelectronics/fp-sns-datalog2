...
uint8_t Ilps22qs_Press_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "ilps22qs_press.sensor_annotation"))
  {
    ...
    json_object_dotset_string(respJSONObject, "ilps22qs_press.sensor_annotation.value", ret == 0 ? sensor_annotation : "PNPL_SET_ERROR");
  }
  json_value_free(tempJSON);
  if (pretty == 1)
  {
...
uint8_t Ilps22qs_Press_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  return 1;
}
