...
/**
  ...
  * dtmi:vespucci:steval_stwinbx1:fpSnsDatalog2_datalog2:sensors:imp34dt05_mic;4
  *
...
uint8_t Imp34dt05_Mic_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  json_object_dotset_number(JSON_Status, "imp34dt05_mic.aop", temp_f);
  int32_t temp_i = 0;
  imp34dt05_mic_get_volume(&temp_i);
  json_object_dotset_number(JSON_Status, "imp34dt05_mic.volume", temp_i);
  bool temp_b = 0;
  ...
  imp34dt05_mic_get_resolution(&temp_f);
  if (temp_f == imp34dt05_mic_resolution_bit16)
  {
    enum_id = 0;
  }
  json_object_dotset_number(JSON_Status, "imp34dt05_mic.resolution", enum_id);
  char *temp_s = "";
...
uint8_t Imp34dt05_Mic_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.odr"))
  {
    ...
    {
      case 0:
        ret = imp34dt05_mic_set_odr(imp34dt05_mic_odr_hz16000);
        break;
      case 1:
        ret = imp34dt05_mic_set_odr(imp34dt05_mic_odr_hz32000);
        break;
      case 2:
        ret = imp34dt05_mic_set_odr(imp34dt05_mic_odr_hz48000);
        break;
    }
    if(ret == 0){
      json_object_dotset_number(respJSONObject, "imp34dt05_mic.odr.value", odr);
    } else {
      json_object_dotset_string(respJSONObject, "imp34dt05_mic.odr.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.volume"))
  {
    ...
    ret = imp34dt05_mic_set_volume(volume);
    if(ret == 0){
      json_object_dotset_number(respJSONObject, "imp34dt05_mic.volume.value", volume);
    } else {
      json_object_dotset_string(respJSONObject, "imp34dt05_mic.volume.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.enable"))
  {
    ...
    ret = imp34dt05_mic_set_enable(enable);
    if(ret == 0){
      json_object_dotset_boolean(respJSONObject, "imp34dt05_mic.enable.value", enable);
    } else {
      json_object_dotset_string(respJSONObject, "imp34dt05_mic.enable.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.samples_per_ts"))
  {
    ...
    ret = imp34dt05_mic_set_samples_per_ts(samples_per_ts);
    if(ret == 0){
      json_object_dotset_number(respJSONObject, "imp34dt05_mic.samples_per_ts.value", samples_per_ts);
    } else {
      json_object_dotset_string(respJSONObject, "imp34dt05_mic.samples_per_ts.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.sensor_annotation"))
  {
    ...
    ret = imp34dt05_mic_set_sensor_annotation(sensor_annotation);
    json_object_dotset_string(respJSONObject, "imp34dt05_mic.sensor_annotation.value", ret == 0 ? sensor_annotation : "PNPL_SET_ERROR");
  }
  json_value_free(tempJSON);
  if (pretty == 1)
  {
...
uint8_t Imp34dt05_Mic_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  return 1;
}
