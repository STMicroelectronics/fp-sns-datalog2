...
uint8_t Imp23absu_Mic_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  imp23absu_mic_get_resolution(&temp_f);
  if (temp_f == imp23absu_mic_resolution_bit16)
  {
    enum_id = 0;
  }
...
uint8_t Imp23absu_Mic_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "imp23absu_mic.sensor_annotation"))
  {
    ...
    json_object_dotset_string(respJSONObject, "imp23absu_mic.sensor_annotation.value", ret == 0 ? sensor_annotation : "PNPL_SET_ERROR");
  }
  json_value_free(tempJSON);
  if (pretty == 1)
  {
...
uint8_t Imp23absu_Mic_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  return 1;
}
