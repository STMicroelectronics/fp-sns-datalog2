...
uint8_t Ism330dhcx_Mlc_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON, char **response, uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "ism330dhcx_mlc.sensor_annotation"))
  {
    ...
    json_object_dotset_string(respJSONObject, "ism330dhcx_mlc.sensor_annotation.value", ret == 0 ? sensor_annotation : "PNPL_SET_ERROR");
  }
  json_value_free(tempJSON);
  if (pretty == 1)
  {
  ...
  if (json_object_dothas_value(tempJSONObject, "ism330dhcx_mlc*load_file.ucf_data"))
  {
    ...
    if (json_object_dothas_value(tempJSONObject, "ism330dhcx_mlc*load_file.ucf_data.data"))
    {
      ...
      if (json_object_dothas_value(tempJSONObject, "ism330dhcx_mlc*load_file.ucf_data.size"))
      {
        ...
        json_object_dotset_string(respJSONObject, "ism330dhcx_mlc*load_file.result", ret == 0 ? "PNPL_CMD_OK" : "PNPL_CMD_ERROR");
      }
    }
  }
  json_value_free(tempJSON);
