...
There is a known limitation in MDF peripheral configuration in STM32CubeMX which generates incomplete code. That's why we strongly suggest not to update the 'mdf.c/.h' files but to use the ones already provided. Overwriting 'mdf.c/.h' files can result in a not working code.
...
The mx folder provided here was tested in FP-SNS-DATALOG2. If you don't need to change the default configuration, the files can be used as they are without any modification.
