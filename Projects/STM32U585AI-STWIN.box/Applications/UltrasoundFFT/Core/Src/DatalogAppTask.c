                                                 ...
                                                 ULONG *pParams)
{
  ...
  if (TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "DatalogApp_Q", nItemSize / 4, pvQueueItemsBuff,
                                    DT_TASK_CFG_IN_QUEUE_LENGTH * nItemSize))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    return res;
  ...
  if (ActivePowerMode == E_POWER_MODE_SENSORS_ACTIVE && NewPowerMode == E_POWER_MODE_STATE1)
  {
    ...
    if (p_obj->datalog_model->log_controller_model.interface == LOG_CTRL_MODE_USB) /*stop command from USB*/
    {
      ...
      for (int32_t ii = 0; ii < SENSOR_NUMBER; ii++)
      {
        ...
        {
          stream_id = p_obj->datalog_model->s_models[ii]->stream_params.stream_id;
      ...
      for (int32_t ii = 0; ii < ALGORITHM_NUMBER; ii++)
      {
        ...
        {
          stream_id = p_obj->datalog_model->a_models[ii]->stream_params.stream_id;
          IStream_dealloc((IStream_t *) p_obj->usbx_device, stream_id);
          p_obj->sensorContext[ii].n_samples_to_timestamp = 0;
          p_obj->sensorContext[ii].old_time_stamp = -1.0f;
        }
...
sys_error_code_t DatalogAppTask_vtblForceExecuteStep(AManagedTaskEx *_this, EPowerMode ActivePowerMode)
{
  ...
  if ((ActivePowerMode == E_POWER_MODE_STATE1) || (ActivePowerMode == E_POWER_MODE_SENSORS_ACTIVE)
      || (ActivePowerMode == E_POWER_MODE_STARTING))
  {
...
sys_error_code_t algorithmEvtListener_ProcessOnNewDataReady_vtbl(IEventListener *_this, const DataEvent_t *pxEvt)
{
  ...
  if (nPMState == E_POWER_MODE_SENSORS_ACTIVE)
  {
    ...
    uint8_t stream_id = p_obj->datalog_model->a_models[aId]->stream_params.stream_id;
...
sys_error_code_t sensorEvtListener_ProcessOnNewDataReady_vtbl(IEventListener *_this, const DataEvent_t *pxEvt)
{
  ...
  if (nPMState == E_POWER_MODE_SENSORS_ACTIVE)
  {
    ...
    {
      p_obj->datalog_model->s_models[sId]->stream_params.ioffset = pxEvt->timestamp; /*TODO: can I use PnPL Setter? no*/
...
uint8_t DatalogAppTask_start_vtbl(ILog_Controller_t *_this, uint32_t interface)
{
  ...
  for (int32_t ii = 0; ii < SENSOR_NUMBER; ii++)
  {
    ...
    {
      stream_id = p_obj->datalog_model->s_models[ii]->stream_params.stream_id;
      usb_ep = p_obj->datalog_model->s_models[ii]->stream_params.usb_ep;
      usb_dps = p_obj->datalog_model->s_models[ii]->stream_params.usb_dps;
      /** use to set ep**/
      ...
      IStream_alloc_resource((IStream_t *) p_obj->usbx_device, stream_id, usb_dps,
                             p_obj->datalog_model->s_models[ii]->comp_name);
    }
  }
  ...
  for (int32_t ii = 0; ii < ALGORITHM_NUMBER; ii++)
  {
    ...
    {
      stream_id = p_obj->datalog_model->a_models[ii]->stream_params.stream_id;
      usb_ep = p_obj->datalog_model->a_models[ii]->stream_params.usb_ep;
      usb_dps = p_obj->datalog_model->a_models[ii]->stream_params.usb_dps;
      /** use to set ep**/
      usbx_dctrl_class_set_ep(p_obj->usbx_device, stream_id, usb_ep);
      IStream_alloc_resource((IStream_t *) p_obj->usbx_device, stream_id, usb_dps,
                             p_obj->datalog_model->s_models[ii]->comp_name);
