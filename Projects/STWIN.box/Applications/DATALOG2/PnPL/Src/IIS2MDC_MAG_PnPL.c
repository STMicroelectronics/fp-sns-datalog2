...
uint8_t Iis2mdc_Mag_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  int32_t temp_i = 0;
  iis2mdc_mag_get_samples_per_ts__val(&temp_i);
  json_object_dotset_number(JSON_Status, "iis2mdc_mag.samples_per_ts.val", temp_i);
  iis2mdc_mag_get_samples_per_ts__min(&temp_i);
  json_object_dotset_number(JSON_Status, "iis2mdc_mag.samples_per_ts.min", temp_i);
  iis2mdc_mag_get_samples_per_ts__max(&temp_i);
  json_object_dotset_number(JSON_Status, "iis2mdc_mag.samples_per_ts.max", temp_i);
...
uint8_t Iis2mdc_Mag_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  {
    if (json_object_dothas_value(tempJSONObject, "iis2mdc_mag.samples_per_ts.val"))
    {
