...
uint8_t Imp34dt05_Mic_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  int32_t temp_i = 0;
  imp34dt05_mic_get_samples_per_ts__val(&temp_i);
  json_object_dotset_number(JSON_Status, "imp34dt05_mic.samples_per_ts.val", temp_i);
  imp34dt05_mic_get_samples_per_ts__min(&temp_i);
  json_object_dotset_number(JSON_Status, "imp34dt05_mic.samples_per_ts.min", temp_i);
  imp34dt05_mic_get_samples_per_ts__max(&temp_i);
  json_object_dotset_number(JSON_Status, "imp34dt05_mic.samples_per_ts.max", temp_i);
...
uint8_t Imp34dt05_Mic_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  {
    if (json_object_dothas_value(tempJSONObject, "imp34dt05_mic.samples_per_ts.val"))
    {
      int32_t samples_per_ts =(int32_t) json_object_dotget_number(tempJSONObject, "imp34dt05_mic.samples_per_ts.val");
      imp34dt05_mic_set_samples_per_ts__val(samples_per_ts);
    }
  }
  json_value_free(tempJSON);
  return ret;
