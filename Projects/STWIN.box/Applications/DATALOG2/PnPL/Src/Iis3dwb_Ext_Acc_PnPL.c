...
/**
  ...
  * dtmi:vespucci:steval_stwinbx1:fpSnsDatalog2_datalog2:sensors:iis3dwb_ext_acc;1
  *
...
uint8_t Iis3dwb_Ext_Acc_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  JSON_Value *tempJSON;
...
uint8_t Iis3dwb_Ext_Acc_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if(json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.fs"))
  {
    ...
    {
    case 0:
      iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g2);
      break;
    case 1:
      iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g4);
      break;
    case 2:
      iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g8);
      break;
    case 3:
      iis3dwb_ext_acc_set_fs(iis3dwb_ext_acc_fs_g16);
      break;
    }
  }
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.enable"))
  {
    ...
    iis3dwb_ext_acc_set_enable(enable);
  }
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.samples_per_ts"))
  {
    ...
    iis3dwb_ext_acc_set_samples_per_ts(samples_per_ts);
  }
  if (json_object_dothas_value(tempJSONObject, "iis3dwb_ext_acc.sensor_annotation"))
  {
    ...
    iis3dwb_ext_acc_set_sensor_annotation(sensor_annotation);
  }
  json_value_free(tempJSON);
  return ret;
}

uint8_t Iis3dwb_Ext_Acc_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON)
{
  return 1;
}
