...
uint8_t Imp23absu_Mic_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  uint8_t enum_id = 0;
  if(temp_f == imp23absu_mic_odr_hz16000)
  {
    enum_id = 0;
  }
  else if(temp_f == imp23absu_mic_odr_hz32000)
  {
    enum_id = 1;
  }
  else if(temp_f == imp23absu_mic_odr_hz48000)
  {
    enum_id = 2;
  }
  else if(temp_f == imp23absu_mic_odr_hz96000)
  {
    enum_id = 3;
  }
  else if(temp_f == imp23absu_mic_odr_hz192000)
  {
    enum_id = 4;
  }
  ...
  int32_t temp_i = 0;
  imp23absu_mic_get_samples_per_ts__val(&temp_i);
  json_object_dotset_number(JSON_Status, "imp23absu_mic.samples_per_ts.val", temp_i);
  imp23absu_mic_get_samples_per_ts__min(&temp_i);
  json_object_dotset_number(JSON_Status, "imp23absu_mic.samples_per_ts.min", temp_i);
  imp23absu_mic_get_samples_per_ts__max(&temp_i);
  json_object_dotset_number(JSON_Status, "imp23absu_mic.samples_per_ts.max", temp_i);
...
uint8_t Imp23absu_Mic_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if(json_object_dothas_value(tempJSONObject, "imp23absu_mic.odr"))
  {
    ...
    {
    case 0:
      imp23absu_mic_set_odr(imp23absu_mic_odr_hz16000);
      break;
    case 1:
      imp23absu_mic_set_odr(imp23absu_mic_odr_hz32000);
      break;
    case 2:
      imp23absu_mic_set_odr(imp23absu_mic_odr_hz48000);
      break;
    case 3:
      imp23absu_mic_set_odr(imp23absu_mic_odr_hz96000);
      break;
    case 4:
      imp23absu_mic_set_odr(imp23absu_mic_odr_hz192000);
      break;
    }
  }
  ...
  {
    if (json_object_dothas_value(tempJSONObject, "imp23absu_mic.samples_per_ts.val"))
    {
