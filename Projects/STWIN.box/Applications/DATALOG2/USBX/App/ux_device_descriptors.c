...
/* USER CODE END PD */
//#define USBD_SERIAL_NUMBER                             "MultiSS002"
...
static uint8_t USBD_FrameWork_SS_WCID_ConfigurationDesc(USBD_DevClassHandleTypeDef *pdev,
                                                        uint8_t Speed,
...
static void USBD_FrameWork_SS_WCID_If_Ep_Desc(USBD_DevClassHandleTypeDef *pdev,
...
/**
  * @brief  USBD_Get_SerialStrDescriptor
  ...
  */
uint8_t USBD_Get_SerialStrDescriptor(UCHAR *buff, uint16_t *length)
{
  *length = USB_SIZ_STRING_SERIAL;

  /* Update the serial number string descriptor with the data from the unique ID*/
  int32_t deviceserial0, deviceserial1, deviceserial2;
  ...
  if (deviceserial0 != 0)
  {
    //      IntToUnicode(deviceserial0, &buff[2], 8);
    //      IntToUnicode(deviceserial1, &buff[18], 4);
    memcpy(&buff[1], &deviceserial0, 4);
    memcpy(&buff[5], &deviceserial1, 2);

  }

  buff[0] = USB_SIZ_STRING_SERIAL;
...
uint8_t *USBD_Get_Device_FS_Descriptor(ULONG *Length)
{
  ...
  USBD_Device_Framework_Builder(&USBD_Device_FS, pDevFrameWorkDesc_FS,
                                UserClassInstance, USBD_FULL_SPEED);
...
uint8_t *USBD_Get_StringDescription_Descriptor(ULONG *Length)
{
  ...
  UCHAR *destser = USBD_string_framework + count;
  USBD_Get_SerialStrDescriptor(destser, &len);


  /*Microsoft OS String Descriptor*/
  count += len + 1;
  USBD_string_framework[count++] = 0x00U;
  USBD_string_framework[count++] =  0x00U;
  ...
  *Length = strlen((const char *)USBD_string_framework);
  ...
  UCHAR *dest = USBD_string_framework + count;
  uint16_t os_size = USBD_OSStringDesc[0];
  for (int i = 0; i < os_size; i++)
  {
    dest[i] = USBD_OSStringDesc[i];
  }
  *Length += USBD_OSStringDesc[0] + 1;
                                                         ...
                                                         uint8_t *pCmpstConfDesc)
{
  ...
  switch (pdev->tclasslist[pdev->classId].ClassType)
  {
    case CLASS_TYPE_SENSOR_STREAMING:
      ...
      pdev->tclasslist[pdev->classId].NumEps = N_IN_ENDPOINTS;

      //FULL SPEED
      /* Assign IN Endpoints */
      USBD_FrameWork_AssignEp(pdev, DATA_IN_EP1, USBD_EP_TYPE_BULK,
                              SS_WCID_DATA_FS_MAX_PACKET_SIZE);

      USBD_FrameWork_AssignEp(pdev, DATA_IN_EP2, USBD_EP_TYPE_BULK,
                              SS_WCID_DATA_FS_MAX_PACKET_SIZE);

      USBD_FrameWork_AssignEp(pdev, DATA_IN_EP3, USBD_EP_TYPE_BULK,
                              SS_WCID_DATA_FS_MAX_PACKET_SIZE);

      USBD_FrameWork_AssignEp(pdev, DATA_IN_EP4, USBD_EP_TYPE_BULK,
                              SS_WCID_DATA_FS_MAX_PACKET_SIZE);

      USBD_FrameWork_AssignEp(pdev, DATA_IN_EP5, USBD_EP_TYPE_BULK,
                              SS_WCID_DATA_FS_MAX_PACKET_SIZE);
                                              ...
                                              uint32_t pConf, uint32_t *Sze)
{
  ...
  pIfDesc->bNumEndpoints = N_IN_ENDPOINTS + 1;
...
//  ptr->bLength                = (uint8_t)sizeof(USBD_ConfigDescTypedef);
//  ptr->bDescriptorType        = USB_DESC_TYPE_CONFIGURATION;
//  ptr->wDescriptorLength      = USB_SS_WCID_CONFIG_DESC_SIZ;
...
////    memcpy(&USBD_StringSerial[1], &deviceserial0, sizeof(uint32_t));
////    memcpy(&USBD_StringSerial[5], &deviceserial1, sizeof(uint32_t));
//
//  }
//}
