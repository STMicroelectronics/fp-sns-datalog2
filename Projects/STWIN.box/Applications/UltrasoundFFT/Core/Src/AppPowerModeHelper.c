...
EPowerMode AppPowerModeHelper_vtblComputeNewPowerMode(IAppPowerModeHelper *_this, const SysEvent event)
{
  ...
  switch(event.xEvent.nSource)
  {
    ...
    case SYS_PM_EVT_SRC_PB:
      ...
      {
        power_mode = E_POWER_MODE_STARTING;
      }
      else if(power_mode == E_POWER_MODE_STARTING)
      {
        power_mode = E_POWER_MODE_STATE1;
...
boolean_t AppPowerModeHelper_vtblCheckPowerModeTransaction(IAppPowerModeHelper *_this, const EPowerMode active_power_mode, const EPowerMode new_power_mode)
{
  ...
  switch(active_power_mode)
  {
    case E_POWER_MODE_STATE1:
      if((new_power_mode == E_POWER_MODE_SENSORS_ACTIVE) || (new_power_mode == E_POWER_MODE_SLEEP_1) || (new_power_mode == E_POWER_MODE_STARTING))
      {
        res = TRUE;
      }
      break;
    ...
    case E_POWER_MODE_STARTING:
      if((new_power_mode == E_POWER_MODE_SENSORS_ACTIVE) || (new_power_mode == E_POWER_MODE_STATE1))
      {
        res = TRUE;
      }
      break;
  ...
  switch(power_mode)
  {
    case E_POWER_MODE_SLEEP_1:
      ...
      if(!SysEventsPending())
      {
        ...
        /* The MCU has exited the STOP mode reset the WWDG*/
        SysResetAEDCounter();
    ...
    case E_POWER_MODE_STARTING:
      SYS_DEBUGF(SYS_DBG_LEVEL_VERBOSE, ("PMH: STARTING SD \r\n"));
      break;
