...
sys_error_code_t FDM_ACOTask_vtblOnCreateTask(AManagedTask *_this, tx_entry_function_t *pTaskCode, CHAR **pName, VOID **pvStackStart, ULONG *pStackDepth,
                                              UINT *pPriority, UINT *pPreemptThreshold, ULONG *pTimeSlice, ULONG *pAutoStart, ULONG *pParams)
{
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "AcoTask_Q", item_size / 4, pvQueueItemsBuff, ACO_TASK_CFG_IN_QUEUE_ITEM_COUNT * item_size))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    sys_error_handler();
...
sys_error_code_t FDM_ACOTask_vtblDoEnterPowerMode(AManagedTask *_this, const EPowerMode eActivePowerMode, const EPowerMode eNewPowerMode)
{
  assert_param(_this);
  sys_error_code_t xRes = SYS_NO_ERROR_CODE;
  FDM_ACOTask *p_obj = (FDM_ACOTask*) _this;
...
sys_error_code_t FDM_ACOTask_vtblOnEnterTaskControlLoop(AManagedTask *_this)
{
  ...
  if(p_obj->mic_sensor_source != NULL)
  {
    ...
    if(IDPU2_AttachToDataSource((IDPU2_t*) p_obj->p_dpu, p_obj->mic_sensor_source, (IDataBuilder_t*) &p_obj->data_builder, E_IDB_NO_DATA_LOSS))
    {
      sys_error_handler();
    }
  }
...
sys_error_code_t FDM_ACOTask_vtblOnEnterPowerMode(AManagedTaskEx *_this, const EPowerMode eActivePowerMode, const EPowerMode eNewPowerMode)
{
  assert_param(_this);
  sys_error_code_t xRes = SYS_NO_ERROR_CODE;
  //  FDM_ACOTask *p_obj = (FDM_ACOTask*)_this;
