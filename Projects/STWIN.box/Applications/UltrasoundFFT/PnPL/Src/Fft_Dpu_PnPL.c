...
uint8_t Fft_Dpu_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
    ...
    fft_dpu_get_packet_size(&temp_i);
    json_object_dotset_number(JSON_Status, "fft_dpu.packet_size", temp_i);
    fft_dpu_get_dim(&temp_i);
    json_object_dotset_number(JSON_Status, "fft_dpu.dim", temp_i);
    fft_dpu_get_fft_length(&temp_i);
    json_object_dotset_number(JSON_Status, "fft_dpu.fft_length", temp_i);
    fft_dpu_get_algorithm_type(&temp_s);
    uint8_t enum_id = 0;
    if(strcmp(temp_s, fft) == 0)
    {
        enum_id = 0;
    }
    json_object_dotset_number(JSON_Status, "fft_dpu.algorithm_type", enum_id);
    fft_dpu_get_fft_sample_freq(&temp_i);
    json_object_dotset_number(JSON_Status, "fft_dpu.fft_sample_freq", temp_i);
    //Next fields are not in DTDL model but Added looking @the component schema field (this is :algorithms) ONLY for Sensors and Algorithms
