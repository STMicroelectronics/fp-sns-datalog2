/**
  ...
  ******************************************************************************
  */
#ifndef LSM6DSV320XTASK_H_
#define LSM6DSV320XTASK_H_
...
#define LSM6DSV320X_MAX_DRDY_PERIOD           (1.0)    /* seconds */

#ifndef LSM6DSV320X_MAX_WTM_LEVEL
#define LSM6DSV320X_MAX_WTM_LEVEL             (145)    /* samples */
#endif

#define LSM6DSV320X_MIN_WTM_LEVEL             (16)     /* samples */
#define LSM6DSV320X_MAX_SAMPLES_PER_IT        (LSM6DSV320X_MAX_WTM_LEVEL)


#define LSM6DSV320X_CFG_MAX_LISTENERS         2

/**
  * Create  type name for _LSM6DSV320XTask.
  */
typedef struct _LSM6DSV320XTask LSM6DSV320XTask;

/**
  *  LSM6DSV320XTask internal structure.
  */
struct _LSM6DSV320XTask
{
  /**
    * Base class object.
    */
  ...
  /**
    ...
    */
  uint8_t p_fast_sensor_data_buff[LSM6DSV320X_MAX_SAMPLES_PER_IT * 7];

  /**
    * Buffer to store the data from the slower subsensor 1
    */
  uint8_t p_medium_sensor_data_buff[LSM6DSV320X_MAX_SAMPLES_PER_IT / 2 * 6];

  /**
      * Buffer to store the data from the slower subsensor 2
      */
  uint8_t p_slow_sensor_data_buff[LSM6DSV320X_MAX_SAMPLES_PER_IT / 3 * 6];
#else
  /**
    * Buffer to store the data read from the sensor FIFO.
    * It is reused also to save data from the faster subsensor
  ...
  /**
    ...
    */
  ULONG lsm6dsv320x_task_cfg_timer_period_ms;
...
/**
  * Allocate an instance of LSM6DSV320XTask in the system heap.
  *
  * @param pIRQConfig [IN] specifies a ::MX_GPIOParams_t instance declared in the mx.h file.
  *        It must be a GPIO connected to the LSM6DSV320X sensor and configured in EXTI mode.
...
/**
  ...
  * The size of the memory block must be greater or equal to `sizeof(LSM6DSV320XTask)`.
  * This allocator allows the application to avoid the dynamic allocation.
  *
  * \code
  * LSM6DSV320XTask sensor_task;
  * LSM6DSV320XTaskStaticAlloc(&sensor_task);
  * \endcode
  *
...
/**
  * IRQ callback
  */
void LSM6DSV320XTask_EXTI_Callback(uint16_t Pin);
