...
sys_error_code_t IMP23ABSUTask_vtblHardwareInit(AManagedTask *_this, void *pParams)
{
  ...
  else
  {
    ...
    {
      MDFDriverFilterRegisterCallback((MDFDriver_t*) p_obj->p_driver, HAL_MDF_ACQ_HALFCOMPLETE_CB_ID, MDF_Filter_1_HalfComplete_Callback);
      MDFDriverFilterRegisterCallback((MDFDriver_t*) p_obj->p_driver, HAL_MDF_ACQ_COMPLETE_CB_ID, MDF_Filter_1_Complete_Callback);
    }
  }
                                                ...
                                                ULONG *pParams)
{
  ...
  *pAutoStart = TX_AUTO_START;
  ...
  if(TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "IMP23ABSU_Q", item_size / 4u, p_queue_items_buff, IMP23ABSU_TASK_CFG_IN_QUEUE_LENGTH * item_size))
  {
    res = SYS_TASK_HEAP_OUT_OF_MEMORY_ERROR_CODE;
    SYS_SET_SERVICE_LEVEL_ERROR_CODE(res);
    return res;
