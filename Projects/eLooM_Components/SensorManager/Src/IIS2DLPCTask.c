  ...
  {
    {
      ...
      IIS2DLPCTask_vtblSensorGetStatusPointer
    },
...
sys_error_code_t IIS2DLPCTask_vtblOnEnterTaskControlLoop(AManagedTask *_this)
{
  ...
  SYS_DEBUGF(SYS_DBG_LEVEL_DEFAULT, ("IIS2DLPC: start.\r\n"));
...
static sys_error_code_t IIS2DLPCTaskSensorInit(IIS2DLPCTask *_this)
{
  ...
  *(uint8_t *) &(int2_route) = 0;
  ...
  {
#if IIS2DLPC_FIFO_ENABLED
    _this->iis2dlpc_task_cfg_timer_period_ms = (uint16_t)((1000.0f / _this->sensor_status.type.mems.odr) * (((float_t)(_this->samples_per_it)) / 2.0f));
#else
