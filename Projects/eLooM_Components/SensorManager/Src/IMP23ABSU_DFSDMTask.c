...
#include "IMP23ABSU_DFSDMTask_vtbl.h"
...
{
  IMP23ABSUTask *p_new_obj = (IMP23ABSUTask *)IMP23ABSUTaskAlloc(p_mx_dfsdm_cfg, p_mx_adc_cfg);
...
AManagedTaskEx *IMP23ABSUTaskStaticAllocSetName(void *p_mem_block, const void *p_mx_dfsdm_cfg, const void *p_mx_adc_cfg,
                                                const char *p_name)
{
  IMP23ABSUTask *p_obj = (IMP23ABSUTask *)IMP23ABSUTaskStaticAlloc(p_mem_block, p_mx_dfsdm_cfg, p_mx_adc_cfg);
  ...
  if (TX_SUCCESS == tx_queue_receive(&p_obj->in_queue, &report, TX_WAIT_FOREVER))
  {
    ...
    switch (report.messageID)
    {
      ...
      case SM_MESSAGE_ID_DATA_READY:
      {
        ...
        {
          p_obj->p_sensor_data_buff[idx] = p_obj->old_out = (0xFC * (p_obj->old_out + ((p_obj->p_dma_data_buff[(p_obj->half - 1) * samples + idx]) >> 12) - p_obj->old_in)) / 0xFF;
          p_obj->old_in = (p_obj->p_dma_data_buff[idx]) >> 12;
        }
      ...
      case SM_MESSAGE_ID_SENSOR_CMD:
      {
        switch (report.sensorMessage.nCmdID)
        {
          case SENSOR_CMD_ID_INIT:
            res = AnalogMicDrvSetDataBuffer((AnalogMicDriver_t *) p_obj->p_driver, p_obj->p_dma_data_buff, ((uint32_t)p_obj->sensor_status.type.audio.frequency / 1000) * 2);
            if (!SYS_IS_ERROR_CODE(res))
            {
