...
AManagedTaskEx *ISM330ISTaskAlloc(const void *pIRQConfig, const void *pISPUConfig, const void *pCSConfig)
{
  ...
  strcpy(sTaskObj.ispu_sensor_status.Name, sTheClass.ispu_class_descriptor.Name);
  ...
  ULONG *pParams)
{
  ...
  else if (TX_SUCCESS != tx_queue_create(&p_obj->in_queue, "ISM330IS_Q", item_size / 4u, p_queue_items_buff,
...
sys_error_code_t ISM330ISTask_vtblSensorSetFifoWM(ISensor_t *_this, uint16_t fifoWM)
{
  ...
  /* Does not support this virtual function.*/
  SYS_SET_SERVICE_LEVEL_ERROR_CODE(SYS_INVALID_FUNC_CALL_ERROR_CODE);
  SYS_DEBUGF(SYS_DBG_LEVEL_ALL, ("ISM330IS: warning - SetFifoWM() not supported.\r\n"));
...
//  ret_val = ism330is_reset_set(p_sensor_drv, 1);
  ...
  _this->ism330is_task_cfg_timer_period_ms = (uint16_t)(
                                               _this->acc_sensor_status.ODR < _this->gyro_sensor_status.ODR ? _this->acc_sensor_status.ODR :
                                               _this->gyro_sensor_status.ODR);
