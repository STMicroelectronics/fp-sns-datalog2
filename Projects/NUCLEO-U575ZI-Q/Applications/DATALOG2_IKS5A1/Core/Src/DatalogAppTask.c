...
//TODO could be more useful to have a CommandParse Class? (ICommandParse + PnPLCommand_t)
  ...
  ISensorLL_t *ism6hg256x_mlc_sensor_ll;
  ISensorLL_t *iis2dulpx_mlc_sensor_ll;
...
uint8_t DatalogAppTask_Set_ISM6HG256XMLC_IF(AManagedTask *task_obj)
{
  DatalogAppTask *p_obj = getDatalogAppTask();
  /* Store SensorLL interface for LSM6DSV16X Sensor */
  p_obj->ism6hg256x_mlc_sensor_ll = ISM6HG256XTaskGetSensorLLIF((ISM6HG256XTask *) task_obj);
  return 0;
}

uint8_t DatalogAppTask_Set_IIS2DULPXMLC_IF(AManagedTask *task_obj)
{
  DatalogAppTask *p_obj = getDatalogAppTask();
  /* Store SensorLL interface for LIS2DUXS12 Sensor */
  p_obj->iis2dulpx_mlc_sensor_ll = IIS2DULPXTaskGetSensorLLIF((IIS2DULPXTask *) task_obj);
  return 0;
}

uint8_t DatalogAppTask_Set_ISM330ISMLC_IF(AManagedTask *task_obj)
{
  DatalogAppTask *p_obj = getDatalogAppTask();
...
uint8_t DatalogAppTask_Set_IIS3DWB10ISExtMLC_IF(AManagedTask *task_obj)
{
  DatalogAppTask *p_obj = getDatalogAppTask();
  /* Store SensorLL interface for IIS3DWB10ISExt Sensor */
...
uint8_t DatalogAppTask_load_ism6hg256x_ucf_vtbl(const char *ucf_data, int32_t ucf_size)
{
  ...
  UCFP_Init(&ucf_protocol, p_obj->ism6hg256x_mlc_sensor_ll);
...
uint8_t DatalogAppTask_load_iis2dulpx_ucf_vtbl(const char *ucf_data, int32_t ucf_size)
{
  ...
  /* Create and initialize a new instance of UCF Protocol service */
  SUcfProtocol_t ucf_protocol;
  UCFP_Init(&ucf_protocol, p_obj->iis2dulpx_mlc_sensor_ll);
  ...
  /* Enable MLC */
  SQInit(&q1, SMGetSensorManager());
  id = SQNextByNameAndType(&q1, "iis2dulpx", COM_TYPE_MLC);
  SMSensorEnable((uint8_t)id);

  /* Get LSM6DSV16X status from SM and update app_model */
  SQInit(&q2, SMGetSensorManager());
  id = SQNextByNameAndType(&q2, "iis2dulpx", COM_TYPE_ACC);
  sensor_status = SMSensorGetStatus(id);
  iis2dulpx_acc_set_samples_per_ts((int32_t)sensor_status.type.mems.odr, NULL);
                                ...
                                int32_t output_size)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (id != 0xFFFF)
      {
        iis2dulpx_mlc_load_file(p_ucf_data, ucf_size);
      }
      else
      {
