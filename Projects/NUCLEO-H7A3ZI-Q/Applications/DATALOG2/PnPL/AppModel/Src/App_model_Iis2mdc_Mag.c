...
/**
  ...
  * dtmi:vespucci:nucleo_h7a3zi_q:x_nucleo_iks02a1:fpSnsDatalog2_datalog2:sensors:iis2mdc_mag;1
  *
...
uint8_t iis2mdc_mag_set_samples_per_ts(int32_t value, char **response_message)
{
  ...
  else if (value > max_v)
  {
    ...
    if (response_message != NULL)
    {
      *response_message = "Error: Value setting above maximum threshold";
    }
  }
  else
  {
    ...
    if (response_message != NULL)
    {
      *response_message = "Error: Value setting below minimum threshold";
    }
  }
  return ret;
}
