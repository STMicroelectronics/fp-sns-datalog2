/**
  ******************************************************************************
  * @file    App_model_Iis2dulpx_Mlc.c
  * @author  SRA
  * @brief   Iis2dulpx_Mlc PnPL Components APIs
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file in
  * the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  *
  ******************************************************************************
  */

/**
  ******************************************************************************
  * This file has been auto generated from the following DTDL Component:
  * dtmi:vespucci:steval_stwinbx1:fpSnsDatalog2_datalog2:sensors:iis2dulpx_mlc;1
  *
  * Created by: DTDL2PnPL_cGen version 2.1.0
  *
  * WARNING! All changes made to this file will be lost if this is regenerated
  ******************************************************************************
  */

#include "App_model.h"
#include "services/SQuery.h"

/* USER includes -------------------------------------------------------------*/

/* USER private function prototypes ------------------------------------------*/

/* USER defines --------------------------------------------------------------*/

/* IIS2DULPX_MLC PnPL Component ----------------------------------------------*/
static SensorModel_t iis2dulpx_mlc_model;
extern AppModel_t app_model;

uint8_t iis2dulpx_mlc_comp_init(void)
{
  iis2dulpx_mlc_model.comp_name = iis2dulpx_mlc_get_key();

  SQuery_t querySM;
  SQInit(&querySM, SMGetSensorManager());
  uint16_t id = SQNextByNameAndType(&querySM, "iis2dulpx", COM_TYPE_MLC);
  iis2dulpx_mlc_model.id = id;
  iis2dulpx_mlc_model.sensor_status = SMSensorGetStatusPointer(id);
  iis2dulpx_mlc_model.stream_params.stream_id = -1;
  iis2dulpx_mlc_model.stream_params.usb_ep = -1;

  addSensorToAppModel(id, &iis2dulpx_mlc_model);
  __stream_control(true);
  /* USER Component initialization code */
  return PNPL_NO_ERROR_CODE;
}

char *iis2dulpx_mlc_get_key(void)
{
  return "iis2dulpx_mlc";
}


uint8_t iis2dulpx_mlc_get_enable(bool *value)
{
  *value = iis2dulpx_mlc_model.sensor_status->is_active;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_samples_per_ts(int32_t *value)
{
  *value = iis2dulpx_mlc_model.stream_params.spts;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_ucf_status(bool *value)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_dim(int32_t *value)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_ioffset(float_t *value)
{
  *value = iis2dulpx_mlc_model.stream_params.ioffset;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_data_type(char **value)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_usb_dps(int32_t *value)
{
  *value = iis2dulpx_mlc_model.stream_params.usb_dps;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_sd_dps(int32_t *value)
{
  *value = iis2dulpx_mlc_model.stream_params.sd_dps;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_sensor_annotation(char **value)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_sensor_category(int32_t *value)
{
  *value = iis2dulpx_mlc_model.sensor_status->isensor_class;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_mounted(bool *value)
{
  *value = true;
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_stream_id(int8_t *value)
{
  *value = iis2dulpx_mlc_model.stream_params.stream_id;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}

uint8_t iis2dulpx_mlc_get_ep_id(int8_t *value)
{
  *value = iis2dulpx_mlc_model.stream_params.usb_ep;
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}


uint8_t iis2dulpx_mlc_set_enable(bool value, char **response_message)
{
  if (response_message != NULL)
  {
    *response_message = "";
  }
  uint8_t ret = PNPL_NO_ERROR_CODE;
  if (value)
  {
    ret = SMSensorEnable(iis2dulpx_mlc_model.id);
  }
  else
  {
    ret = SMSensorDisable(iis2dulpx_mlc_model.id);
  }
  if (ret == SYS_NO_ERROR_CODE)
  {
    /* USER Code */
    __stream_control(true);
    /* USER Code */
  }
  return ret;
}

uint8_t iis2dulpx_mlc_set_sensor_annotation(const char *value, char **response_message)
{
  if (response_message != NULL)
  {
    *response_message = "";
  }
  uint8_t ret = PNPL_NO_ERROR_CODE;
  strcpy(iis2dulpx_mlc_model.annotation, value);
  return ret;
}


uint8_t iis2dulpx_mlc_load_file(const char *data, int32_t size)
{
  /* USER Code */
  return PNPL_NO_ERROR_CODE;
}


