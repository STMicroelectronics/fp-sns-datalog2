...
uint8_t Lps22hh_Press_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  int32_t temp_i = 0;
  lps22hh_press_get_odr(&temp_i);
  uint8_t enum_id = 0;
  if (temp_i == lps22hh_press_odr_hz1)
  {
    enum_id = 0;
  }
  else if (temp_i == lps22hh_press_odr_hz10)
  {
    enum_id = 1;
  }
  else if (temp_i == lps22hh_press_odr_hz25)
  {
    enum_id = 2;
  }
  else if (temp_i == lps22hh_press_odr_hz50)
  {
    enum_id = 3;
  }
  else if (temp_i == lps22hh_press_odr_hz75)
  {
    enum_id = 4;
  }
  else if (temp_i == lps22hh_press_odr_hz100)
  {
    enum_id = 5;
  }
  else if (temp_i == lps22hh_press_odr_hz200)
  {
    enum_id = 6;
  }
  json_object_dotset_number(JSON_Status, "lps22hh_press.odr", enum_id);
  lps22hh_press_get_fs(&temp_i);
  enum_id = 0;
  if (temp_i == lps22hh_press_fs_hpa1260)
  {
    enum_id = 0;
  }
                                           ...
                                           uint32_t *size, uint8_t pretty)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "lps22hh_press.odr"))
  {
    ...
    {
      case 0:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz1);
        break;
      case 1:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz10);
        break;
      case 2:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz25);
        break;
      case 3:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz50);
        break;
      case 4:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz75);
        break;
      case 5:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz100);
        break;
      case 6:
        ret = lps22hh_press_set_odr(lps22hh_press_odr_hz200);
        break;
    }
    if (ret == 0)
    {
      json_object_dotset_number(respJSONObject, "lps22hh_press.odr.value", odr);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "lps22hh_press.odr.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "lps22hh_press.enable"))
  {
    ...
    ret = lps22hh_press_set_enable(enable);
    if (ret == 0)
    {
      json_object_dotset_boolean(respJSONObject, "lps22hh_press.enable.value", enable);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "lps22hh_press.enable.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "lps22hh_press.samples_per_ts"))
  {
    ...
    ret = lps22hh_press_set_samples_per_ts(samples_per_ts);
    if (ret == 0)
    {
      json_object_dotset_number(respJSONObject, "lps22hh_press.samples_per_ts.value", samples_per_ts);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "lps22hh_press.samples_per_ts.value", "PNPL_SET_ERROR");
    }
  }
  if (json_object_dothas_value(tempJSONObject, "lps22hh_press.sensor_annotation"))
  {
    ...
    ret = lps22hh_press_set_sensor_annotation(sensor_annotation);
    if (ret == 0)
    {
      json_object_dotset_string(respJSONObject, "lps22hh_press.sensor_annotation.value", sensor_annotation);
    }
    else
    {
      json_object_dotset_string(respJSONObject, "lps22hh_press.sensor_annotation.value", "PNPL_SET_ERROR");
    }
  }
  json_value_free(tempJSON);
  if (pretty == 1)
