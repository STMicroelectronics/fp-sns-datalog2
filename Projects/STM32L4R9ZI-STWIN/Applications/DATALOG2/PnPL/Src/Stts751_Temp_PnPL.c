...
/**
  ...
  * dtmi:appconfig:steval_stwinkt1b:fpSnsDatalog2_datalog2:sensors:stts751_temp;1
  *
...
uint8_t Stts751_Temp_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  float temp_f = 0;
  stts751_temp_get_odr(&temp_f);
  uint8_t enum_id = 0;
  if(temp_f == stts751_temp_odr_hz1)
  {
    enum_id = 0;
  }
  else if(temp_f == stts751_temp_odr_hz2)
  {
    enum_id = 1;
  }
  else if(temp_f == stts751_temp_odr_hz4)
  {
    enum_id = 2;
  }
  else if(temp_f == stts751_temp_odr_hz8)
  {
    enum_id = 3;
  }
  json_object_dotset_number(JSON_Status, "stts751_temp.odr", enum_id);
  stts751_temp_get_fs(&temp_f);
  enum_id = 0;
  if(temp_f == stts751_temp_fs_cdeg100)
  {
    enum_id = 0;
  }
...
uint8_t Stts751_Temp_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if(json_object_dothas_value(tempJSONObject, "stts751_temp.odr"))
  {
    ...
    {
    case 0:
      stts751_temp_set_odr(stts751_temp_odr_hz1);
      break;
    case 1:
      stts751_temp_set_odr(stts751_temp_odr_hz2);
      break;
    case 2:
      stts751_temp_set_odr(stts751_temp_odr_hz4);
      break;
    case 3:
      stts751_temp_set_odr(stts751_temp_odr_hz8);
      break;
    }
  }
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.enable"))
  {
    ...
    stts751_temp_set_enable(enable);
  }
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.samples_per_ts"))
  {
    ...
    stts751_temp_set_samples_per_ts(samples_per_ts);
  }
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.sensor_annotation"))
  {
    ...
    stts751_temp_set_sensor_annotation(sensor_annotation);
  }
  json_value_free(tempJSON);
  return ret;
}

uint8_t Stts751_Temp_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON)
{
  return 1;
}
