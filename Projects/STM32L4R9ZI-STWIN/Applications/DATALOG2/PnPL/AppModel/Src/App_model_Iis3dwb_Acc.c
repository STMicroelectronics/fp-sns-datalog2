...
uint8_t iis3dwb_acc_comp_init(void)
{
  ...
  iis3dwb_acc_set_st_ble_stream__acc_unit("g", NULL);
  iis3dwb_acc_set_st_ble_stream__acc_format("int16_t", NULL);
  ...
  iis3dwb_acc_set_st_ble_stream__acc_channels(value, NULL);
  float_t sensitivity = 0.0f;
  iis3dwb_acc_get_sensitivity(&sensitivity);
  iis3dwb_acc_set_st_ble_stream__acc_multiply_factor(sensitivity, NULL);
...
{
  float_t odr = iis3dwb_acc_model.sensor_status->type.mems.odr;
...
{
  float_t fs = iis3dwb_acc_model.sensor_status->type.mems.fs;
  if (fs < 3.0f)
  {
...
uint8_t iis3dwb_acc_get_ioffset(float_t *value)
{
...
uint8_t iis3dwb_acc_get_measodr(float_t *value)
{
...
uint8_t iis3dwb_acc_get_sensitivity(float_t *value)
{
...
uint8_t iis3dwb_acc_get_st_ble_stream__acc_multiply_factor(float_t *value)
{
  /* USER Code */
...
uint8_t iis3dwb_acc_set_st_ble_stream__acc_multiply_factor(float_t value, char **response_message)
{
  if (response_message != NULL)
  {
    *response_message = "";
