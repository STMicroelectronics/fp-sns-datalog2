...
/**
  ...
  * dtmi:vespucci:steval_stwinkt1b:fpSnsDatalog2_datalog2:sensors:stts751_temp;3
  *
...
uint8_t Stts751_Temp_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  pnpl_stts751_temp_odr_t temp_odr_e = (pnpl_stts751_temp_odr_t)0;
  stts751_temp_get_odr(&temp_odr_e);
  json_object_dotset_number(JSON_Status, "stts751_temp.odr", temp_odr_e);
  pnpl_stts751_temp_fs_t temp_fs_e = (pnpl_stts751_temp_fs_t)0;
  stts751_temp_get_fs(&temp_fs_e);
  json_object_dotset_number(JSON_Status, "stts751_temp.fs", temp_fs_e);
  bool temp_b = 0;
  ...
  stts751_temp_get_st_ble_stream__id(&temp_i);
  json_object_dotset_number(JSON_Status, "stts751_temp.st_ble_stream.id", temp_i);
  stts751_temp_get_st_ble_stream__temp_enable(&temp_b);
  json_object_dotset_boolean(JSON_Status, "stts751_temp.st_ble_stream.temp.enable", temp_b);
  stts751_temp_get_st_ble_stream__temp_unit(&temp_s);
  json_object_dotset_string(JSON_Status, "stts751_temp.st_ble_stream.temp.unit", temp_s);
  stts751_temp_get_st_ble_stream__temp_format(&temp_s);
  json_object_dotset_string(JSON_Status, "stts751_temp.st_ble_stream.temp.format", temp_s);
  stts751_temp_get_st_ble_stream__temp_elements(&temp_i);
  json_object_dotset_number(JSON_Status, "stts751_temp.st_ble_stream.temp.elements", temp_i);
  stts751_temp_get_st_ble_stream__temp_channels(&temp_i);
  json_object_dotset_number(JSON_Status, "stts751_temp.st_ble_stream.temp.channels", temp_i);
  stts751_temp_get_st_ble_stream__temp_multiply_factor(&temp_f);
  json_object_dotset_number(JSON_Status, "stts751_temp.st_ble_stream.temp.multiply_factor", temp_f);
  stts751_temp_get_st_ble_stream__temp_odr(&temp_i);
  json_object_dotset_number(JSON_Status, "stts751_temp.st_ble_stream.temp.odr", temp_i);
                                          ...
                                          uint32_t *size, uint8_t pretty)
{
  ...
  {
    int32_t odr = (int32_t)json_object_dotget_number(tempJSONObject, "stts751_temp.odr");
    valid_property = true;
    ret = stts751_temp_set_odr((pnpl_stts751_temp_odr_t)odr, &resp_msg);
    ...
    {
      pnpl_stts751_temp_odr_t old_odr;
      stts751_temp_get_odr(&old_odr);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.enable"))
  {
    ...
    valid_property = true;
    ret = stts751_temp_set_enable(enable, &resp_msg);
    json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
    if (ret == PNPL_NO_ERROR_CODE)
    {
    ...
    {
      bool old_enable;
      stts751_temp_get_enable(&old_enable);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.samples_per_ts"))
  {
    ...
    valid_property = true;
    ret = stts751_temp_set_samples_per_ts(samples_per_ts, &resp_msg);
    json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
    if (ret == PNPL_NO_ERROR_CODE)
    {
    ...
    else
    {
      ...
      stts751_temp_get_samples_per_ts(&old_samples_per_ts);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.sensor_annotation"))
  {
    ...
    valid_property = true;
    ret = stts751_temp_set_sensor_annotation(sensor_annotation, &resp_msg);
    json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
    if (ret == PNPL_NO_ERROR_CODE)
    {
    ...
    else
    {
      ...
      stts751_temp_get_sensor_annotation(&old_sensor_annotation);
      json_object_dotset_string(respJSONObject, "PnPL_Response.value", old_sensor_annotation);
      json_object_dotset_boolean(respJSONObject, "PnPL_Response.status", false);
    }
  }
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.id"))
    {
      int32_t st_ble_stream__id = (int32_t)json_object_dotget_number(tempJSONObject, "stts751_temp.st_ble_stream.id");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__id(st_ble_stream__id, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__id(&old_st_ble_stream__id);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.enable"))
    {
      bool st_ble_stream__temp_enable = json_object_dotget_boolean(tempJSONObject, "stts751_temp.st_ble_stream.temp.enable");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_enable(st_ble_stream__temp_enable, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__temp_enable(&old_st_ble_stream__temp_enable);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.unit"))
    {
      const char *st_ble_stream__temp_unit = json_object_dotget_string(tempJSONObject,
                                                                       "stts751_temp.st_ble_stream.temp.unit");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_unit(st_ble_stream__temp_unit, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__temp_unit(&old_st_ble_stream__temp_unit);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.format"))
    {
      const char *st_ble_stream__temp_format = json_object_dotget_string(tempJSONObject,
                                                                         "stts751_temp.st_ble_stream.temp.format");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_format(st_ble_stream__temp_format, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__temp_format(&old_st_ble_stream__temp_format);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.elements"))
    {
      int32_t st_ble_stream__temp_elements = (int32_t)json_object_dotget_number(tempJSONObject,
                                                                                "stts751_temp.st_ble_stream.temp.elements");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_elements(st_ble_stream__temp_elements, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__temp_elements(&old_st_ble_stream__temp_elements);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.channels"))
    {
      int32_t st_ble_stream__temp_channels = (int32_t)json_object_dotget_number(tempJSONObject,
                                                                                "stts751_temp.st_ble_stream.temp.channels");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_channels(st_ble_stream__temp_channels, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__temp_channels(&old_st_ble_stream__temp_channels);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.multiply_factor"))
    {
      float_t st_ble_stream__temp_multiply_factor = (float_t)json_object_dotget_number(tempJSONObject,
                                                    "stts751_temp.st_ble_stream.temp.multiply_factor");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_multiply_factor(st_ble_stream__temp_multiply_factor, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      {
        float_t old_st_ble_stream__temp_multiply_factor;
        stts751_temp_get_st_ble_stream__temp_multiply_factor(&old_st_ble_stream__temp_multiply_factor);
  ...
  if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream"))
  {
    if (json_object_dothas_value(tempJSONObject, "stts751_temp.st_ble_stream.temp.odr"))
    {
      int32_t st_ble_stream__temp_odr = (int32_t)json_object_dotget_number(tempJSONObject,
                                                                           "stts751_temp.st_ble_stream.temp.odr");
      valid_property = true;
      ret = stts751_temp_set_st_ble_stream__temp_odr(st_ble_stream__temp_odr, &resp_msg);
      json_object_dotset_string(respJSONObject, "PnPL_Response.message", resp_msg);
      if (ret == PNPL_NO_ERROR_CODE)
      {
      ...
      else
      {
        ...
        stts751_temp_get_st_ble_stream__temp_odr(&old_st_ble_stream__temp_odr);
  ...
  else
  {
    ...
    char *log_message = "Invalid property for stts751_temp";
