...
int32_t hci_tl_spi_receive(uint8_t *buffer, uint16_t size)
{
  ...
  /* Read the header */
  HAL_SPI_TransmitReceive(&hspi1, header_master, header_slave, HEADER_SIZE, 1000);
  ...
  if (byte_count > 0)
  {
    ...
    {
      HAL_SPI_TransmitReceive(&hspi1, &char_ff, (uint8_t *)&read_char, 1, 1000);
  ...
  do
  {
    ...
    */
    ...
    if (rx_bytes >= size)
    {
      ...
      HAL_SPI_TransmitReceive(&hspi1, buffer, read_char_buf, size, 1000);
    }
    else
    {
      /* Buffer is too small */
