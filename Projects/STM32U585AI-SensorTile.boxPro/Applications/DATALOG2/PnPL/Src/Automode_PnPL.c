...
/**
  ...
  * dtmi:appconfig:other:automode;1
  *
...
uint8_t Automode_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  automode_get_start_delay_ms(&temp_i);
  json_object_dotset_number(JSON_Status, "automode.start_delay_ms", temp_i);
  automode_get_datalog_time_length(&temp_i);
  json_object_dotset_number(JSON_Status, "automode.datalog_time_length", temp_i);
  automode_get_idle_time_length(&temp_i);
  json_object_dotset_number(JSON_Status, "automode.idle_time_length", temp_i);
...
uint8_t Automode_PnPL_vtblSetProperty(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "automode.start_delay_ms"))
  {
    int32_t start_delay_ms =(int32_t) json_object_dotget_number(tempJSONObject, "automode.start_delay_ms");
    automode_set_start_delay_ms(start_delay_ms);
  }
  if (json_object_dothas_value(tempJSONObject, "automode.datalog_time_length"))
  {
    int32_t datalog_time_length =(int32_t) json_object_dotget_number(tempJSONObject, "automode.datalog_time_length");
    automode_set_datalog_time_length(datalog_time_length);
  }
  if (json_object_dothas_value(tempJSONObject, "automode.idle_time_length"))
  {
    int32_t idle_time_length =(int32_t) json_object_dotget_number(tempJSONObject, "automode.idle_time_length");
    automode_set_idle_time_length(idle_time_length);
  }
  json_value_free(tempJSON);
  return ret;
}
